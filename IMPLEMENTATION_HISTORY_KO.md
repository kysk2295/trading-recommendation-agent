# 상세 구현 히스토리: 기관형 다중 시장 Quant Research OS
...
일반 v1 config 거부는 유지하며, 이미 로드된 exact v1 current pair를 검증해 v2 candidate로 내리는
one-way replace만 별도 읽기 전용 경계에서 허용한다. v1 candidate나 v1 재기동은 허용하지 않는다.
...
**2026-07-24 systematic 독립 Reviewer 업데이트:** query-only Reviewer가 exact global trial registration/terminal chain, persisted card, content-addressed completed-daily source와 그 source에서 다시 계산한 shadow outcome을 대사하고 terminal의 card/source/outcome SHA 집합을 검증한다. 별도 mode-600 append-only ledger는 completed에 `continue_collection`, missed-session censor에 `data_quality_review`만 기록하며 자동 상태, 주문, allocation 권한은 모두 false다. actual 2026-07-24 trial은 아직 terminal 전이라 preflight review 0건이고, exact `5c78ee9` frozen runtime을 7월 24·27일 16:05 EDT finalizer 성공 receipt 뒤 각각 16:12 EDT에 실행하도록 예약했다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m6-systematic-independent-reviewer-schedule-ko.md)에 있다.

**2026-07-24 intraday code-version bootstrap 업데이트:** 새 code commit 뒤 clean main `701b354`를 다음 NYSE open 전 local-only global experiment ledger에 사전등록했다. 기존 exact 가설 네 개는 재사용하고 code-coupled strategy version·authority·lifecycle을 각각 4건 append했으며 exact replay는 신규 0건이었다. account/broker/order mutation은 0건이고 이는 `experimental_shadow` identity일 뿐 Paper champion이나 주문 권한이 아니다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-intraday-code-version-bootstrap-701b354-ko.md)에 있다.

**2026-07-24 KR M3 code rollover·예약 업데이트:** 기존 exact KR Opportunity/day hypothesis와 base version/policy를 재검증한 뒤 clean `caf5901` code SHA의 두 shadow version을 한 transaction으로 append했다. 새 mode-600 policy/bundle의 exact replay는 version 0건을 추가했다. 별도 at-most-once `ai.trading-agent.kr-m3-20260724` chain은 08:55 KST calendar부터 15:32 terminal·Reviewer까지 예약됐다. 이 실행 당시 OpenDART 설정이 없었으므로 source 실패를 완화하지 않고 `CENSORED` data-quality evidence로만 닫는다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-kr-m3-code-rollover-schedule-ko.md)에 있다.
...
**2026-07-26 인터랙티브 에이전트 대시보드 업데이트:** Observatory를 `개요 / 에이전트 / 계좌·PnL / 추천·근거` 작업 탭으로 재구성하고 에이전트별 목표 입력, queued/running/completed/failed 원장과 최종 응답을 이벤트 기반으로 연결했다. 공개 조회에는 계속 키가 필요 없고 명령은 Mac publisher가 발급한 일회용 링크로 연결된 Secure HttpOnly 기기 세션에서만 가능하다. 브라우저와 publisher는 주기적 HTTP polling을 하지 않으며 명시적 명령 1건만 로컬 Hermes 1회 실행으로 이어진다. 명령과 응답은 공개 viewer에 전송되지 않고 실제 자금 주문 권한도 추가하지 않는다. 실제 Railway 배포와 production 명령 왕복은 [체크포인트](docs/checkpoints/2026-07-26-interactive-agent-observatory-ko.md)에 있다.
...
**2026-07-24 M8 closeout bounded-wait 업데이트:** strict closeout이 05:20 KST까지 receipt를 게시할 수 있는데 cumulative actual research가 05:18에 한 번만 확인하던 예약 race를 TDD로 수정했다. planned actual-research CLI는 timezone-aware deadline까지 receipt와 report의 atomic publication을 bounded poll한 뒤 기존 strict result·300~390 동일 cycle cardinality를 그대로 검증한다. 전체 `3612 passed`, Ruff, basedpyright `0/0`과 delayed-publication CLI happy path를 통과했고 exact `9678318`을 `origin/main`에 push했다. 실행 전이던 05:18 예약은 receipt/claim/출력 0을 확인한 뒤 05:22 KST at-most-once label로 교체했으며 미국장 수집·후속 audit·Hermes는 유지했다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m8-closeout-prerequisite-wait-readiness-ko.md)에 있다.

**2026-07-24 BLS Public Data v1 거시 source 업데이트:** 벤치마킹 source coverage의 첫 추가 단위로 무등록 공식 BLS 다중 시계열 POST를 raw-first·append-only collector에 연결했다. 1~25개 대문자 series와 최대 10년 범위를 exact request identity로 고정하고, HTTP·content type·series cardinality·관측 순서를 검증한 content-addressed snapshot과 capability assessment를 mode 600으로 보존한다. exact `ad20351339c623522ef55b12162a0a36e451d04a` runtime의 실제 CPI-U·실업률 요청은 36개 관측 중 34개를 수치로 받았고, BLS가 2025년 10월 연방정부 셧다운 결측으로 공식 표기한 각주가 있는 `-` 두 건은 보간하지 않고 `null`로 보존해 `9444 bps/degraded`로 기록했다. 최초 공식 POST는 network `1`, exact replay는 network `0`, receipt/run `1/1`, artifact `yes/no`였고 전체 `3623 passed`, Ruff, basedpyright `0/0`을 통과했다. 상세 근거와 아직 자격증명·라이선스가 필요한 source는 [체크포인트](docs/checkpoints/2026-07-24-m6-bls-public-macro-source-ko.md)에 있다.

**2026-07-24 FRED·ALFRED point-in-time 거시 source 업데이트:** owner-only mode-600 key file을 쓰는 공식 FRED series observations GET을 raw-first collector에 연결하고, 최신 `fred/series_observations`와 명시적 vintage의 `alfred/vintage_observations`를 별도 capability로 등록했다. ALFRED 요청은 vintage를 request identity에 고정하고 응답의 모든 real-time period가 그 날짜와 같아야만 성공하므로 현재 수정값을 과거 연구 입력으로 섞지 않는다. exact `d1aae55443bc0a2be3bdfe781586418abfeabc87` runtime에서 `CPIAUCSL`의 2024년 1~3월을 실제 조회한 결과 최신 FRED는 3개, 2024-04-01 ALFRED vintage는 당시 존재한 2개 관측만 보존했다. 최초 두 공식 GET은 각각 network `1`, 존재하지 않는 credential을 지정한 exact replay는 network `0`과 동일 snapshot bytes였고, evidence 21개는 모두 mode 600이며 key 포함 파일은 0개였다. 두 capability는 `10000 bps/complete`, 전체 `3628 passed`, Ruff, basedpyright `0/0`을 통과했다. 이후 exact `d574cc9` runtime에서 현재 mode-600 key를 다시 검증해 DFF 최신 22건과 `2026-07-23` ALFRED vintage 22건을 각각 공식 GET으로 수집했고, 두 capability 모두 `10000 bps/complete`, missing-key exact replay network `0`을 유지했다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m6-fred-alfred-point-in-time-source-ko.md)에 있다.

**2026-07-24 ALFRED multi-vintage revision panel 업데이트:** 검증된 ALFRED snapshot 2~100개를 query-only로 읽어 vintage별 `not_observed`·공식 `missing`·`available`과 직전 available 값 대비 Decimal revision을 보존하는 content-addressed panel을 추가했다. 동일 series·units·관측 범위와 canonical filename/body, vintage 인과성을 모두 대사하고 최신값으로 과거 공백을 채우지 않는다. 실제 DFF `2026-07-22/23` 두 vintage는 관측 `21/22`, panel 22행, 비교 가능한 revision 21개, 변경 revision 0개였으며 새 release 관측 1개는 revision이 아니라 availability 변화로 보존됐다. exact `42a42f8` replay는 provider·credential 접근 0, artifact 신규 생성 없음, 전체 `3649 passed`, Ruff와 basedpyright `0/0`을 통과했다. 이어 exact `7538e8b`에서 공식 `fred/series/vintagedates` raw-first capability를 연결해 실제 DFF change date 16개를 수집했고, exact panel ID와 calendar ID를 함께 대사한 release admission은 두 vintage `2/2 READY`였다. exact `dd00368`에서는 최신 FRED·ALFRED·공식 change-calendar 세 capability와 panel/calendar/admission의 정확한 파일 SHA를 하나의 `3/3 READY` Macro Revision Data Foundation으로 결합했다. foundation ID는 `13e1b5343dfcc1a3efe00d54ffa26c8e0c03c65faa30819c3b91d586080c8ca5`, first/committed replay artifact는 `yes/no`, network·credential 접근은 `0/0`이었다. 전체 `3654 passed`, Ruff와 basedpyright `0/0`을 통과했다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m6-alfred-revision-panel-ko.md)에 있다.

**2026-07-24 Alpaca News actual source 업데이트:** 기존 bounded raw-first collector를 실제 Market Data `GET /v1beta1/news`에 연결했다. 10개 명시 심볼·50분 창의 최초 GET은 HTTP success raw 1 page/534 bytes와 article metadata 1건을 보존했고, 존재하지 않는 credential 경로의 exact replay는 network `0`이었다. local capability는 `complete`, coverage는 `10/10`, `10000 bps`, point-in-time evidence snapshot은 10개였으며 coverage/evidence exact replay는 신규 artifact `0/0`이었다. exact `c36816c` code-coupled news-catalyst shadow baseline을 사전등록해 actual evidence를 실행했지만 최근 5분 provider update 후보가 없어 terminal `no_candidates`, eligible 0으로 정직하게 닫았다. 계좌·주문·포지션 endpoint와 mutation은 0건이다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m6-alpaca-news-actual-source-ko.md)에 있다.
...
**2026-07-23 KR M3 finalizer 실증:** 예약된 15:32 KST 장후 control cycle이 terminal·Hermes delivery·독립 Reviewer·lifecycle 네 phase를 모두 성공으로 닫았다. source preflight 결손을 0수익으로 바꾸지 않고 session을 `censored/no_shadow_entry_artifact`, Reviewer를 `data_quality_review`로 보존했으며 terminal/review 원장은 반복 실행 뒤에도 각각 한 행이었다. 이는 운영 control-plane 실증이지 clean source session, 추천 성과, champion 또는 주문·allocation 권한이 아니다.

**2026-07-23 Alpaca option-chain M6 업데이트:** underlying·만기·call/put·`indicative|opra` feed·page budget을 명시한 exact Alpaca option-chain GET을 추가했다. HTTP 오류를 포함한 wire bytes를 parser 전에 append-only mode-600 receipt로 확정하고, OCC 계약 identity와 latest quote/trade·IV·Greeks를 검증한 immutable terminal을 만든다. exact replay는 network를 다시 열지 않으며 local capability registry는 `US_DERIVATIVES`의 historical research와 shadow use만 허용한다. trading/account/order API는 이 경로에 없고 real-time·OPRA entitlement를 추정하지 않는다. clean SHA `388f099369402b04f38070e7dee140de21bc5bcd`의 AAPL indicative 정규장 smoke는 2026-07-23 09:40 EDT에 at-most-once로 예약됐으며, 예약 상태 자체는 live 성공·시장 전체 coverage·추천 또는 주문 권한이 아니다.

**2026-07-23 Alpaca option-contract security master M6 업데이트:** Paper origin의 exact `GET /v2/options/contracts`를 underlying·만기·call/put·page budget으로 제한하고 raw bytes를 parser 전에 append-only mode-600 receipt로 보존한다. provider contract UUID와 underlying asset UUID, OCC identity, exercise style, strike, size·multiplier, OI·as-of date를 검증해 canonical `US_DERIVATIVES` option master를 만들며 exact replay는 credential과 network를 열지 않는다. clean SHA `a6029ef2e4d42ccf2c77bb5d1297880320e0ad8f`의 actual AAPL GET은 HTTP 200 raw 1 page에서 고유 계약 77개와 OI 68개를 materialize했고, 존재하지 않는 credential 경로의 replay도 성공했다. 이 GET-only 증거는 시장 전체 coverage, OPRA entitlement, snapshot 성공, 파생상품 전략 성과 또는 주문 권한이 아니다.

**2026-07-23 exact option surface M6 업데이트:** 별도 SQLite에 보존된 option-contract master와 option-chain terminal을 query-only로 읽어 OCC provider symbol, canonical contract UUID, underlying UUID, 만기·right·strike를 exact 대사하는 shadow-only surface를 추가했다. chain snapshot에 대응하는 master identity가 없으면 산출물 공개 전에 차단하고, master 일부에 snapshot이 없으면 삭제하지 않은 채 `DEGRADED` coverage로 보존한다. 전 계약이 결합된 경우에만 `READY`이며 OI·quote/trade·IV·Greeks와 두 input run의 canonical SHA-256을 content-addressed mode-600 JSON에 고정한다. 구현 SHA `194818d630ae720e62c7c0bf62fa65d460e73fc5`의 fixture CLI는 READY 100% coverage, 최초 생성과 exact replay를 통과했다. 실제 22:40 chain smoke의 성공 receipt를 선행조건으로 읽는 별도 22:50 KST at-most-once surface 검증도 예약했다. 이 bounded 단일 underlying·만기·right slice는 IV surface·skew·term structure, 성과, 추천 또는 주문 권한이 아니다.

**2026-07-23 actual option-chain bar 복구 업데이트:** 예약된 chain smoke의 HTTP 200 raw receipt에는 77개 snapshot과 함께 `minuteBar`, `dailyBar`, `prevDailyBar`가 있었지만 strict provider model이 세 필드를 unknown으로 거부해 `response_structure`로 닫혔다. exact `7f5043cb5870871e07f2ebf451b94a9d591eca2c`는 세 OHLCV bar를 timezone-aware typed evidence로 검증·보존한다. 같은 정규장 actual GET은 raw 42,869 bytes, snapshot 77개, minute/daily/previous-daily bar `75/75/71`로 성공했고 실제 77-contract master와 exact identity `77/77`, coverage `10,000 bps`의 `READY` surface를 만들었다. 존재하지 않는 credential 경로의 exact replay도 추가 network 없이 receipt/run `1/1`을 유지했다. 전체 `3478 passed`, Ruff, basedpyright가 통과했고 account/order mutation은 0건이다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-23-m6-option-chain-actual-bar-recovery-ko.md)에 있다.

**2026-07-24 multi-expiration option term structure M6 업데이트:** READY surface 2~32개를 같은 underlying·feed, 고유 만기/right, 최대 300초 관측 skew로 결합하는 query-only term structure를 추가했다. 각 slice는 exact surface semantic ID와 file SHA, OI 관측일·총 OI, IV 관측 수와 all-strike median IV를 보존하며 renamed/non-content-addressed, DEGRADED, 다른 feed·underlying, 2개 미만 만기, stale skew와 OI/IV 결손은 발행 전에 차단한다. exact `15ee93c72aa25ede8bed2c7805cafe50cb12c942` 정규장 actual GET은 AAPL call 2026-07-24 `77/77`과 2026-07-31 `78/78` READY surface를 3.487562초 skew로 결합해 OI `147,531/74,397`, median IV `0.4052/0.3996`을 발행했다. nonexistent credential replay는 contract/chain receipt·run `2/2`, surface `2`, term `1`을 그대로 유지했고 전체 `3483 passed`, Ruff, basedpyright가 통과했다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m6-option-term-structure-actual-ko.md)에 있다.

**2026-07-24 actual option IV term context 업데이트:** READY term structure의 exact semantic ID·file SHA와 단일 option right·고유 만기를 다시 검증한 뒤 near-minus-far median IV를 `front_premium|flat|back_premium`으로 분류하는 query-only Derivatives Research context를 추가했다. 실제 AAPL call 1일/8일 만기의 median IV `0.4052/0.3996`에서 차이 `+0.0056`, `front_premium`을 content-addressed mode-600 artifact로 발행했고 exact replay는 신규 artifact `0`이었다. 전체 `3637 passed`, Ruff, basedpyright `0/0`을 통과했다. 이는 기술적 상태 feature이지 방향·추천·성과·주문 권한이 아니며 SIP가 막힌 actual skew를 우회하지 않는다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m6-option-iv-term-context-actual-ko.md)에 있다.

**2026-07-24 source-backed option skew M6 업데이트:** 같은 underlying·feed·만기의 READY call/put surface와 exact Alpaca SIP canonical completed-minute receipt를 결합해 matched-strike put-minus-call IV와 사전등록 absolute-delta bucket skew를 만드는 shadow-only CLI를 추가했다. spot 숫자를 CLI에서 직접 받지 않고 private runtime SQLite의 receipt, raw lineage와 canonical OHLCV를 다시 대사하며 surface 관측 뒤 receipt, 미래·전일 bar, 300초 초과 surface skew와 private-store 위반을 차단한다. fixture는 strike·delta bucket, content-addressed mode-600 artifact와 exact replay를 통과했다. actual `5dccbb26226a53ea092245a27b6db5f3577153a0`에서 AAPL 2026-07-24 indicative call/put master·chain을 각각 77개 수집해 exact `77/77`, coverage `10000 bps`, IV·Greeks 각 20개의 READY surface를 만들었고 관측 skew는 `1.2354초`였다. nonexistent credential replay는 contract·chain receipt/run `2/2`와 두 surface file SHA를 그대로 유지하며 network·account·order mutation 0건으로 끝났다. 그러나 같은 runtime의 SIP GET은 HTTP `403`을 반환해 `blocked_source/sip_access_forbidden`으로 닫혔고 실제 skew artifact는 만들지 않았다. 같은 SHA의 SIP 재검증을 2026-07-24 09:31:30 EDT one-shot으로 예약했다. 품질 gate, Paper·계좌·주문·allocation 권한은 열지 않았다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m6-source-backed-option-skew-ko.md)에 있다.

**2026-07-24 futures roll security master M6 업데이트:** 동일 root·venue·currency·timezone·multiplier와 provider namespace를 공유하는 2~32개 futures 계약의 stable identity, alias, listing·observation, settlement·first-notice·last-trade와 연속 active/roll window를 검증하는 provider-neutral local vertical을 추가했다. private manifest를 query-only로 읽고 as-of active 계약이 정확히 하나일 때만 content-addressed mode-600 master를 발행하며 exact replay는 같은 artifact를 재사용한다. fixture happy/replay는 artifact `yes/no`, 파일 수 `1`, 동일 SHA를 통과했고 public input은 output 생성 전에 차단됐다. 외부 network와 broker/account/order mutation은 0이다. 이는 실제 CME·ICE licensed source, 현재 계약 coverage, basis·curve·roll 성과 또는 주문 권한이 아니다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m6-futures-roll-security-master-ko.md)에 있다.

**2026-07-24 KIS 해외선물 actual quote source 업데이트:** 공식 KIS GET-only 해외선물 현재가 origin/path/TR과 같은 root의 2~8개 명시 계약을 고정하고, raw response를 parser 전에 append-only mode-600 SQLite에 보존하는 source adapter를 추가했다. exact `f9f09ec` frozen runtime에서 `ESU26/ESZ26`을 요청했지만 첫 계약이 bounded server retry 뒤 HTTP 500, provider code `EGW00550`과 “CME SUB 거래소 신청 계좌가 아님”으로 닫혀 raw receipt/run `1/1`, canonical quote `0`이었다. 두 번째 계약이나 다른 source로 바꾸지 않았고 nonexistent credential replay는 network `0`, 같은 terminal을 재사용했다. 전체 `3641 passed`, Ruff, basedpyright `0/0`을 통과했다. 실제 CME quote와 basis·curve 전에 KIS 계정·앱의 CME SUB 시세 신청이 필요하다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m6-kis-overseas-futures-source-ko.md)에 있다.

**2026-07-24 CFTC TFF positioning context M6 업데이트:** 공식 CFTC Public Reporting Environment의 TFF futures-only dataset을 고정 origin·dataset·field selection과 한 contract market code, 기준일, 최신 두 행으로 제한하는 raw-first vertical을 추가했다. HTTP status를 포함한 최대 1 MiB 응답을 parser 전에 mode-600 append-only SQLite에 확정하고 market·report identity, 날짜·단위, non-negative position과 category long/short open-interest 합계를 검증한 뒤 dealer, asset manager, leveraged money, other reportable, nonreportable net·주간 변화·OI 대비 bps를 content-addressed context로 발행한다. 2026-07-24 actual ES 공개 GET은 최신 report date `2026-07-14`의 두 행과 다섯 category를 통과해 artifact SHA `318619090a28443c33f49771b33c2fab2bdec9ec440eb75dd1b293eb2d19d10d`를 만들었고, exact replay는 receipt/run `1/1`, 추가 network `0`, artifact 생성 `no`였다. 전체 `3523 passed`, Ruff, basedpyright `0/0`, compileall이 통과했다. 이는 market-level weekly aggregate source evidence이며 계약 월별 curve, 실시간성, 전략 성과, Paper 또는 allocation 권한이 아니다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m6-cftc-tff-positioning-context-ko.md)에 있다.

**2026-07-24 futures positioning as-of join M6 업데이트:** immutable CFTC context, futures roll master와 명시적 reviewed binding을 각각 private query-only로 읽어 canonical bytes·content-addressed filename·semantic ID·file SHA를 대사하고, binding 관측·효력, CFTC observation/report freshness와 exact `active_from <= as_of < roll_at` 계약을 모두 통과할 때만 active contract에 결합한다. actual CFTC ES artifact와 reviewed fixture ES roll master를 `2026-07-24T18:00:00Z` 기준으로 결합해 active `cme:es-202609`를 선택했고 context ID `6feb7cdb19de41f04c180afa4be0c2b6aff68c30bfaaf416473d9f961964510b`, file SHA `b0f972561296764a9707ef3c45aad3d962e161ebd83ca4b6d01da93b5d2c45b5`를 mode 600으로 발행했다. exact replay는 artifact `yes/no`, 동일 SHA였고 aggregate report에는 instrument·provider symbol·position·경로를 쓰지 않았다. 전체 `3540 passed`, Ruff, basedpyright `0/0`, compileall, no-excuse가 통과했다. roll master는 licensed current CME source가 아닌 검토 fixture이므로 실제 계약 coverage, curve·basis, 전략 성과, Paper 또는 allocation 권한이 아니다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m6-futures-positioning-context-ko.md)에 있다.

**2026-07-24 Treasury yield curve macro context M6 업데이트:** 미국 재무부 공식 Daily Treasury Par Yield Curve XML feed를 fixed HTTPS origin·path와 request date에서 계산한 한 달 query로 제한하는 raw-first vertical을 추가했다. HTTP status와 최대 1 MiB wire bytes를 parser 전에 mode-600 append-only SQLite에 확정하고 Atom/XML namespace, 고유 날짜와 14개 CMT maturity를 strict하게 검증해 최신 두 곡선의 yield 변화와 `10Y-2Y`, `10Y-3M`, `30Y-5Y` slope를 content-addressed context로 발행한다. exact `4bd7361819445afc61282fecb3fe3c97c22f9a82` actual GET은 HTTP 200 raw `23,848 bytes`, 최신/이전 `2026-07-22/21`, maturity `14`, receipt/run `1/1`을 만들었고 context ID `3fc449d92749b6182aa429fa7baae6bc4a593097bf05ea1a89609b42630cb3cd`, artifact SHA `c4b0ab0197bdc088d67ca801b888506e1838976ee815361a3f2ec35273c7e36a`였다. replay provenance 표기 결함은 `eb2663d32a965b410e1aceca65f35c76a4301777`에서 고쳤다. 같은 frozen runtime의 07:05 KST one-shot도 실제 exit `0`으로 실행돼 HTTP 200 raw `25,393 bytes`, 최신/이전 `2026-07-23/22`, maturity `14`, receipt/run `1/1`과 artifact SHA `a1af4b879a9cdac55cebc0ae8636bc900531c00737712b9c1141e4d8229a3ea9`을 남긴 뒤 label을 제거했다. nonexistent fixture replay는 network `0`, 동일 SHA와 cardinality를 유지했다. 전체 `3554 passed`, Ruff, basedpyright `0/0`, compileall, no-excuse가 통과했다. 이는 공식 일별 par curve이지 intraday tradable quote, 장기 revision coverage, 전략 성과, Paper 또는 allocation 권한이 아니다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m6-treasury-yield-curve-context-ko.md)에 있다.

**2026-07-24 Alpaca SIP entitlement admission 업데이트:** 실제 2026-07-23 SIP smoke가 인증 control 단계에서 받은 공식 provider code 409를 append-only stream DB에서 query-only로 다시 읽어 `blocked/insufficient_subscription` machine admission으로 투영했다. bounded-complete terminal만 `ready`, 402·406·transport·protocol failure와 증거 부재는 entitlement를 추정하지 않고 `unknown`으로 남긴다. exact `00af3d336f7aca0a774eecf6cefe85f12b7179cd` actual artifact ID는 `662777904311eff640d792b160d95202c40f90911de50beb7d26327cd48c8770`, file SHA는 `9c83bc67001f6c5e2ff2199e76c7cf10ac7f9c621113ebfb70afe17c8026985e`였고 input DB SHA·mtime은 실행 전후 같았다. replay artifact는 `1→1`, network/account/order mutation은 0건이며 전체 `3488 passed`, Ruff, basedpyright가 통과했다. 같은 409를 무시하던 7월 24일 future SIP smoke는 provider 접근 전에 제거하고 exact `9cfe56a` at-most-once query-only admission을 09:34 EDT로 교체했다. 이는 현재 SIP trade stream 차단 증거이며 REST 완료 분봉이나 Paper 권한을 승인·부정하지 않는다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-alpaca-sip-entitlement-admission-actual-ko.md)에 있다.
...
**2026-07-20 SEC EDGAR submissions read-only 업데이트:** M5의 첫 US regulatory source로 exact `https://data.sec.gov/submissions/CIK##########.json` GET을 추가했다. 선언된 연락처 `User-Agent`, no redirect, hard 45초 전체 deadline과 wire·decoded 각각 64 MiB cap을 강제하며 0-byte HTTP 오류를 포함한 success/error wire bytes와 content encoding을 parser 전에 별도 mode-600 append-only SQLite receipt로 확정한다. raw 64 MiB 상한은 built-in HTTP·fixture뿐 아니라 typed response와 store public boundary에서 실제 buffer 크기로 다시 검증하고 exact `bytes`로 canonicalize해 custom fetcher나 `bytes` subtype이 우회할 수 없다. pinned `ijson` pure-Python streaming backend는 recent parallel column과 additional-history 목록을 item 2,001에서 Pydantic 전에 중단하고, Pydantic `max_length=2000`과 ignored extra를 2차 방어로 유지한다. rejected suffix가 10배 커져도 parser peak가 2배 미만인 allocation 회귀와 4,500자리 integer가 signal 종료 없이 sanitized `response_structure`가 되는 subprocess 회귀를 고정했다. recent filing은 document CIK와 response issuer CIK가 같고 accession 형식과 문서 identity가 유효한 경우에 accession별 immutable linear version chain으로 보존한다. accession 앞 10자리는 filing agent CIK일 수 있으므로 issuer CIK와의 일치를 강제하지 않는다. correction parent는 mutable SQLite `rowid`가 아니라 자식이 없는 유일한 chain tip으로 선택한다. 동일 terminal 또는 orphan receipt replay는 fixture·User-Agent·provider client를 열기 전에 복구한다. receiptless terminal은 transport failure만 허용하고 receipt-backed failure code는 exact raw response와 일치해야 하며, 모든 receipt-backed run은 `started_at <= received_at <= completed_at`을 만족해야 한다. receiptless transport terminal key에는 늦은 receipt를 추가하지 않고 재시도는 새 collection ID로만 한다. DB/report alias·broken DB symlink·symlinked DB parent·foreign version-0 SQLite·unsafe path는 fixture·User-Agent·provider client와 store mutation 전에 거부한다. CLI lookup은 current-owner mode-700이 아닌 기존 DB parent도 source 구성 전에 거부하며, direct store writer는 기존 공통 정책대로 owner parent를 mode 700으로 강화한 뒤 로컬 write를 수행한다. SQLite main file은 retained parent descriptor에서 `O_NOFOLLOW`로 먼저 열고 non-creating `mode=rw/ro` URI만 사용하며 parent와 file inode를 연결 직후·commit 전후까지 대사한다. 표준 SQLite VFS sidecar까지 dirfd에 결박할 수는 없으므로 runtime은 mode-700 parent를 단독 소유하는 전용 OS identity와 untrusted same-UID code가 없는 writer 구간을 전제로 한다. 이 경계가 깨지면 runtime을 중지하고 trusted evidence에서 재구축한다. SQLite structural integrity, exact DDL·foreign key·raw hash·run/receipt/observation lineage·전체 correction ancestor와 시각 순서를 재검증한다. caller snapshot과 저장된 ordered observation도 exact raw receipt의 deterministic projection과 일치해야 하며, 모든 public write 전에 기존 전체 evidence graph를 먼저 재생한다. fixture success·correction·HTTP failure·transport failure와 CLI happy/replay, focused SEC **85 tests**, SEC+OpenDART **137 tests**, 전체 **2938 tests**, Ruff, basedpyright 0/0를 통과했으며 회사명·accession·CIK·원문·User-Agent는 report에 쓰지 않는다. 이 최초 체크포인트에서는 `filings.files`를 개수로만 기록했고, 바로 아래 bounded additional-history 체크포인트가 해당 선언을 typed manifest와 실제 history 수집으로 확장했다. 최초 체크포인트의 실제 SEC GET과 broker/account/order mutation은 0건이다.

**2026-07-20 SEC EDGAR bounded additional-history 업데이트:** recent raw receipt의 `filings.files`를 파일명·CIK·건수·시작일·종료일이 고정된 typed manifest로 승격하고, exact `https://data.sec.gov/submissions/{declared-file-name}` GET만 허용하는 별도 CLI를 추가했다. 한 invocation은 recent GET과 합쳐도 SEC 10 requests/second 경계 아래인 최대 8개 history 파일만 선언 순서대로 처리하며 첫 terminal failure에서 중단한다. 각 child collection ID는 부모 receipt와 exact manifest의 canonical hash로 결정되고, child run payload는 부모 receipt와 manifest를 보존한다. 기존 SQLite schema를 변경하지 않으면서 store는 부모 recent success run·raw receipt·manifest membership·CIK·child ID·provider receipt 시각·terminal 순서를 모든 append와 replay에서 다시 대사한다. history raw response는 파싱 전에 기존 mode-600 append-only receipt에 확정되고 top-level parallel filing column, manifest count/date 범위, issuer CIK와 accession 형식이 유효할 때만 기존 accession correction chain에 canonical observation을 추가한다. accession prefix는 filing agent CIK일 수 있다. HTTP/구조 실패 raw, transport terminal, parse 이전 orphan과 완료 terminal은 provider·fixture·User-Agent를 다시 열지 않고 복구한다. typed manifest 도입 전 저장된 recent run은 payload에 `source_kind`가 실제로 없을 때만 구형 opaque `filings.files` count와 filing projection을 재생하며 새 수집은 계속 strict manifest를 요구한다. CLI의 파일·설정 실패와 해석할 수 없는 `~user` 경로는 로컬 경로나 traceback을 전달하지 않는 고정 오류로 닫는다. fixture manifest도 no-follow descriptor 하나에서 64 KiB까지 읽고 descriptor와 최종 경로 inode가 같을 때만 파싱한다. 실제 PEP 723 CLI에서 recent fixture `2/2`, history fixture `1/1`, missing User-Agent provider-free replay `1/0`을 확인했다. focused SEC **117 tests**, 전체 **2970 tests**, Ruff와 basedpyright 0/0가 통과했으며 실제 SEC production GET과 broker/account/order mutation은 0건이다.

**2026-07-21 SEC EDGAR capability registry 업데이트:** recent parent와 부모 manifest가 선언한 모든 additional-history child를 SEC 원장의 단일 query-only snapshot에서 재생해 `sec/edgar_submissions` capability와 entitlement로 투영한다. recent를 포함한 각 선언 slice 중 성공한 비율만 completeness로 계산하며 전부 성공하면 `complete`, 미수집 child가 있으면 `incomplete`, terminal 실패 child가 있으면 `degraded`, recent 자체가 실패하면 `failed`로 보존한다. `historical_from`은 manifest의 선언 시작일이 아니라 실제 검증된 filing observation의 가장 이른 날짜다. universe는 `us_equities:bounded_issuer`로 고정해 한 기업 수집을 미국시장 전체 coverage로 과장하지 않는다. local-only CLI는 source ledger를 query-only로 읽고 append-only registry와 mode-600 redacted report만 쓰며 provider·credential·broker를 import하지 않는다. SEC focused **136 tests**, 전체 **2989 tests**, Ruff와 basedpyright 0/0가 통과했고 실제 SEC production GET과 broker mutation은 0건이다. 다음 M5 경계는 filing document 본문과 issuer/company-announcement evidence다.

**2026-07-21 SEC filing document raw-first 업데이트:** 검증된 recent·additional-history filing version에서 최대 8개 최신 accession을 골라 issuer CIK folder, dashless accession, primary document로 exact `https://www.sec.gov/Archives/edgar/data/...` GET target을 만든다. 사용자가 URL이나 document path를 직접 주입할 수 없고, accession prefix가 filing agent CIK여도 archive issuer folder는 metadata issuer CIK를 사용한다. 전용 no-redirect client는 선언된 SEC User-Agent, hard request deadline, 자동 transport retry 없는 64 MiB wire cap을 적용하고 HTTP 오류와 빈 body까지 parser 이전에 별도 mode-600 append-only SQLite receipt로 보존한다. receipt와 terminal run은 exact DDL·foreign key·hash·target·시간·HTTP failure projection과 전체 row 집합을 매번 검증하며 UPDATE/DELETE trigger로 닫힌다. terminal replay와 receipt-only crash recovery는 fixture·User-Agent·provider를 열기 전에 끝나고, batch는 최대 8개를 순차 처리해 첫 실패에서 중단한다. DB·report alias와 invalid report path는 source나 provider를 열기 전에 거부하고 redacted CLI report에는 건수·byte·replay만 기록한다. fixture 프로세스 QA에서 1건/35 bytes 수집과 missing User-Agent provider-free replay를 확인했다. SEC focused **158 tests**, 전체 **3011 tests**, Ruff와 basedpyright 0/0가 통과했으며 production SEC GET, credential read, broker mutation은 0건이다. 다음 M5 경계는 issuer/company-announcement evidence다.

**2026-07-21 Alpaca News raw-first 업데이트:** 회사별 임의 RSS 대신 고정 endpoint와 이용권 계약이 있는 첫 US news source로 exact `https://data.alpaca.markets/v1beta1/news` GET을 추가했다. request는 최대 50개 명시 symbol, 최대 24시간, 페이지당 50건·최대 8페이지로 제한하고 redirect·자동 retry를 금지하며 전체 45초 deadline과 wire 8 MiB cap을 적용한다. HTTP 오류를 포함한 raw bytes를 parser 전에 별도 mode-600 append-only SQLite receipt로 확정하고, terminal replay는 fixture·credential·network보다 먼저 끝내며 receipt-only crash는 다음 page부터 복구한다. strict parser는 request symbol 교집합·timestamp causality·HTTPS URL·page token과 provider article ID 중복을 검증하고 licensed content/summary는 normalized metadata에 복제하지 않는다. local-only capability CLI는 성공한 bounded window만 `alpaca/news`, `us_equities:bounded_symbols`, complete로 투영하고 uses는 historical research와 shadow forward만 허용하며 real-time·Paper recommendation·redistribution은 열지 않는다. fixture 프로세스에서 1 page/331 bytes/1 article metadata 수집, missing credential provider-free replay와 registry idempotent replay를 확인했다. focused **38 tests**, 전체 **3049 tests**, Ruff와 basedpyright 0/0가 통과했으며 production News GET, credential read와 broker/account/order mutation은 0건이다. 상세 계약은 [체크포인트](docs/checkpoints/2026-07-21-alpaca-news-raw-first-ko.md)에 있다.

**2026-07-23 issuer-direct announcement onboarding 업데이트:** M5의 회사 직접 발표 source를 임의 RSS URL이 아닌 private onboarding 계약으로 열었다. onboarding은 exact `issuer_direct/*` source, issuer·symbol, HTTPS endpoint와 item-link allow-list, 이용약관 URL, 자동 접근 허용, 365일 이내 권리 검토, 유효기간, raw/derived retention·삭제·append-correction, 무재배포와 request rate를 모두 고정한다. collector는 redirect·압축을 받지 않는 GET 하나를 45초·2 MiB로 제한하고 HTTP 응답 원문을 reversible base64 mode-600 immutable receipt로 XML parser 전에 확정한다. RSS 2.0과 Atom parser는 DTD/entity, 중복 provider ID, 미래 timestamp, foreign host와 비정상 URL을 차단하고 headline 원문 대신 SHA-256만 terminal evidence에 남긴다. exact terminal 또는 receipt-only crash replay는 fixture·provider보다 먼저 끝나며 capability는 `us_equities:bounded_issuer`, historical research·shadow forward, non-real-time·no redistribution만 선언한다. reserved-domain fixture에서 raw-first 실패 보존, success/replay, registry idempotence와 redacted mode-600 report를 실제 PEP 723 CLI로 확인했다. 저장소에는 production issuer endpoint나 그 권리를 임의 등록하지 않았고 actual provider GET은 0건이다. 상세 계약은 [체크포인트](docs/checkpoints/2026-07-23-m5-issuer-announcement-raw-first-ko.md)에 있다.

**2026-07-21 Alpaca News bounded coverage·Opportunity evidence 업데이트:** 1~8개의 exact news request를 동일 window와 중복 없는 최대 400개 symbol scope로 선언하는 manifest, cutoff 시점의 success/failed/missing assessment와 symbol-weighted completeness를 추가했다. cutoff 뒤 terminal은 hindsight를 막기 위해 missing으로 취급하고 failed partial article은 accepted evidence에서 제외한다. 모든 declared symbol이 success일 때만 symbol별 Opportunity 입력 snapshot을 만들며, 뉴스 0건도 완전한 source에서 관측된 명시적 0건으로 보존한다. snapshot은 headline·URL·본문 대신 raw receipt에 결박된 opaque observation ID와 provider/received timestamp만 포함하고 ranking·추천·전략·주문을 만들지 않는다. coverage와 evidence는 content-addressed mode-600 immutable artifact로 게시되고 exact replay는 created `0/0`이다. 실제 CLI QA에서 complete `2/2`, incomplete `1/2`와 evidence 차단을 확인했다. Alpaca News focused **52 tests**, 전체 **3063 tests**, Ruff와 basedpyright 0/0가 통과했으며 network·credential·broker mutation은 0건이다. gzip/deflate raw receipt replay도 decoded 8 MiB cap으로 수정했다. 상세 계약은 [체크포인트](docs/checkpoints/2026-07-21-alpaca-news-coverage-opportunity-evidence-ko.md)에 있다.
...
**2026-07-21 US news-catalyst current-session feature observation 업데이트:** 공유 intraday kernel의 READY snapshot에 최신 완료 종가를 추가하고 blocked snapshot에서는 노출하지 않도록 했다. runtime fleet의 optional `--news-catalyst-feature-root`는 current-session binding의 close, VWAP, RVOL, prior-high breakout과 research/volume-profile lineage를 content-addressed mode-600 artifact로 저장한다. 별도 local-only observation CLI는 frozen treatment/control 전체가 같은 최신 cycle에 있고, 2분 이내이며, latest completed source가 cohort 관측 30분 뒤 이상인 경우에만 setup manifest를 게시한다. 종목 누락·중복·stale·다른 session은 partial 성과로 바꾸지 않고 차단한다. actual CLI에서 help `0`, missing cohort `1/not-published`, fixture happy/replay `0/0`, artifact mode `600`을 확인했고 focused **17 tests**, 전체 **3102 tests**, Ruff, basedpyright 0/0, changed-file no-excuse audit가 통과했다. provider·credential·account·order operation은 0건이다. 현재 fleet export는 활성 scanner 종목만 대상으로 하므로 frozen cohort 전체를 보장하는 전용 collector와 일일 scheduler는 아직 남아 있다. 상세 계약은 [체크포인트](docs/checkpoints/2026-07-21-us-news-catalyst-feature-observation-ko.md)에 있다.

**2026-07-21 US news-catalyst frozen cohort collector 업데이트:** 활성 scanner scope와 독립적으로 READY cohort의 treatment/control 전체를 exact Alpaca security-master instrument ID에 해석하고, cohort 관측 30분 뒤 2분 창에서 동일 완료 분까지 Alpaca SIP GET-only로 수집하는 전용 collector를 추가했다. 각 instrument는 이전 20개 적격 session의 exact through-minute volume profile과 current-session raw page·canonical minute-bar identity를 보존한다. 수집 전에 cohort, security snapshot, completed minute, symbol/instrument/profile hash를 content-addressed mode-600 plan으로 동결하며, 전체 feature가 READY일 때만 단일 immutable receipt를 발행한다. partial HTTP 실패 뒤에는 저장된 raw/profile을 재생해 누락 instrument만 다시 GET하고, 완성 receipt 재생은 provider와 자격증명을 열지 않는다. actual CLI에서 help `0`, missing input `1/not-published`, fixture happy/replay `0/0`, 4종목 최초 `84 GET`, 재생 추가 GET `0`, credential read `1→0`, receipt mode `600`을 확인했다. 전체 **3109 tests**, Ruff, basedpyright 0/0, changed-file no-excuse audit가 통과했다. 이는 신호·주문·수익성 또는 production 정규장 smoke가 아니며 일일 single-writer scheduler는 다음 경계다. 상세 계약은 [체크포인트](docs/checkpoints/2026-07-21-us-news-catalyst-frozen-cohort-collection-ko.md)에 있다.

**2026-07-21 US news-catalyst 일일 single-writer scheduler 업데이트:** 장전 `register`, 유효한 장중 projection의 `start`, cohort 관측 30분을 엄격히 초과한 뒤 2분 이내 `collect`·`observe`, terminal `finalize`, 장후 독립 `review`를 immutable day-session manifest 하나로 연결했다. 한 tick은 비차단 file lease를 전체 실행 동안 잡고 phase 하나만 처리한다. 완료 판단은 scheduler 상태가 아니라 experiment ledger와 cohort·collection receipt·setup manifest·outcome·review의 content hash를 매번 다시 검증하며, child commit 뒤 audit 기록 전에 중단돼도 다음 tick이 명령을 재실행하지 않고 `recovered` attestation을 append한다. 창을 놓친 collection·observation은 성공으로 가장하지 않고 deterministic `skipped` evidence를 남긴 뒤 trial을 censored terminal로 확정하며, 장후 재시작은 최대 24시간 안에 누락 terminal을 복구할 수 있다. exact `+30:00`에는 끝 시각이 현재와 같은 bar를 사용하지 않고 waiting하며 `+30:01`부터 수집한다. fixture E2E는 여섯 tick에서 실제 child CLI, 4종목 `84 GET`, feature·receipt·observation·terminal·review를 만들고 일곱 번째 tick의 추가 command `0`을 확인했다. 실제 CLI QA는 help `0`, redacted missing input `1`, init/recovery/waiting `0/0/0`, manifest·audit·report mode `600`, provider·account·order mutation `0`이었다. 전체 **3126 tests**, Ruff, basedpyright 0/0와 no-excuse audit가 통과했다. production 정규장 실행·상시 launchd 등록·실표본·수익성·lifecycle 승격은 아직 열지 않았다. 상세 계약은 [체크포인트](docs/checkpoints/2026-07-21-us-news-catalyst-day-session-scheduler-ko.md)에 있다.
...
**2026-07-24 OpenDART actual source 재검증:** 새 mode-600 credential로 별도 read-only 공시검색을 실행해 현재 공식 응답의 `report_nm` 경계 공백과 KR instrument v2 영숫자 종목코드를 발견했다. raw receipt는 그대로 보존하고 canonical 제목만 trim하며 기존 공통 v2 심볼 계약을 재사용하도록 TDD로 수정했다. exact `6413af7` runtime의 재검증은 HTTP 200 3페이지, receipt/catalyst/observation `3/272/272`, source `success`였고, 존재하지 않는 credential 경로의 exact replay는 신규 `0/0/0`이었다. 이는 오늘 09:05 실패 cycle을 소급 변경하지 않으며 7월 27일 four-source cycle의 사전 source 실증이다. 전체 `3646 passed`, Ruff와 basedpyright `0/0`을 통과했다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-opendart-actual-source-revalidation-ko.md)에 있다.
...
**2026-07-23 strict session 누적 catalog 업데이트:** 날짜별 forward-session을 기존 replay 품질 게이트로 먼저 독립 감사하고, 결과·추천 수와 무관하게 clean session만 최대 60개 누적하는 local-only CLI를 추가했다. blocked session은 삭제하거나 0수익으로 바꾸지 않고 reason code와 함께 content-addressed mode-600 catalog receipt에 보존하며 minimum clean floor 미달은 dataset publication 전에 차단한다. 예약일을 `required_session_date`로 고정하면 당일이 blocked인데 과거 clean session만 재평가하는 것도 차단한다. 선택된 집합은 기존 strict materializer를 다시 통과한다. 실제 기존 네 세션은 모두 blocked이고 data artifact는 0건이었다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-23-m8-strict-session-catalog-ko.md)에 있다.

**2026-07-23 actual research coordinator 업데이트:** candidate forward-session catalog부터 exact causal CSV/receipt, actual entitlement·queue 결속, 전략별 READY foundation/v2 manifest, bounded walk-forward와 독립 Reviewer까지 한 fail-closed CLI로 연결했다. required current session이 blocked이면 strategy version/trial mutation 전에 멈추며 exact replay는 artifact를 늘리지 않는다. actual-shaped fixture CLI는 첫 실행 foundation/trial/review `1/1/1`, Reviewer `hold`, replay 신규 trial/review `0/0`이었고 모든 산출물이 mode 600이었다. 이는 실제 시장 성과가 아니라 예약된 clean session을 동일 tested surface로 처리하기 위한 운영 수직축이다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-23-m8-actual-research-coordinator-ko.md)에 있다.

**2026-07-24 M8 intraday equal-risk comparison evidence 업데이트:** 2~3개 completed walk-forward와 독립 review를 global experiment ledger에서 다시 읽어 trial terminal, exact artifact, 재계산한 Reviewer 전체와 같은 causal CSV·persisted manifest·evaluator·비용·session/fold cardinality·현재 data/cost/portfolio contract를 대사하는 query-only comparison CLI를 추가했다. 각 candidate가 20 OOS sessions·30 trades를 채우기 전에는 `collecting`이고 동일 조건의 2개 이상이 하한을 채울 때만 `comparison_ready`이며 승자·champion이나 lifecycle·주문·allocation 권한은 만들지 않는다. exact `2737e111a710013c6caac4b7eaf125e4b0a0fd60` fixture first/replay는 candidate `3`, blocker `6`, artifact `yes/no`, 파일 수 `1`과 동일 SHA를 보존했다. 이어 exact `3cfd7c7e966a75e08ae0c471aab46853837d2faa`에서 actual terminal audit schema v2가 두 개 이상 전략의 comparison artifact ID·상태를 같은 terminal artifact에 결속하고, 단일 전략은 `not_applicable`으로 유지한다. 전체 `3583 passed`, Ruff, basedpyright `0/0`이 통과했다. 예약된 frozen runtime에는 소급되지 않으며 clean-session trial 뒤 이 SHA 이상 runtime으로 다시 audit해야 한다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m8-equal-risk-comparison-evidence-ko.md)에 있다.

**2026-07-24 M8 walk-forward OOS outcome trace 업데이트:** 집계치만 남기던 `intraday_walk_forward_v1`과 분리해 evaluator·experiment schema v2를 추가했다. 각 OOS session의 날짜와 gross/net trade return block, bootstrap sample·seed를 immutable artifact에 보존하고 trade 수·평균·PF·누적수익·MDD·거래일 bootstrap CI와 편도 cost 관계를 trace에서 재계산한다. trace와 집계를 함께 바꾼 위조도 차단하며 legacy v1 artifact는 무재작성으로 읽고 재생한다. actual input binding은 신규 v2 evaluator를 사용한다. 실제 CLI first/replay는 experiment/review `3/3 → 0/0`, schema set `{2}`, mode `600`, stdout/stderr 0바이트였고 전체 `3585 passed`, Ruff, basedpyright `0/0`이 통과했다. 이는 DSR/PBO 통과가 아니라 그 진단에 필요한 exact OOS 입력을 보존한 단계다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m8-walk-forward-outcome-trace-ko.md)에 있다.

**2026-07-24 M8 intraday DSR/PBO 진단 업데이트:** exact completed experiment·독립 review·global ledger를 query-only로 다시 대사하고 schema v2의 OOS session return matrix에서 DSR과 CSCV-PBO를 재계산하는 content-addressed mode-600 CLI를 추가했다. 같은 intraday lane의 reviewed-at 이하 `historical_replay` 등록을 completed 여부와 무관하게 보수적 trial count로 세고, 세 전략·20개 동기 session·전략별 30 trades·동일 크기 CSCV partition·유일 Sharpe 선택이 하나라도 없으면 수치를 만들지 않고 `collecting`으로 닫는다. 20-session 고정 fixture는 DSR `0.9949874120895517`, 4-partition 6-combination PBO `0.0`을 재현했고 값 위조를 차단했다. repository 1-session CLI는 candidate/trial `3/3`, blocker `4`, DSR/PBO `none/none`, first/replay artifact `1/1`로 닫혔다. `diagnostic_ready`도 승격 판정이 아니며 lifecycle·주문·allocation 권한은 모두 false다. 전체 `3591 passed`, Ruff, basedpyright `0/0`이 통과했고 기능 SHA는 `b88f12e2320f44c180d688a2075dbf0b1219f3c2`다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m8-intraday-overfit-diagnostics-ko.md)에 있다.

**2026-07-24 M8 actual terminal DSR/PBO audit 업데이트:** actual terminal audit schema v3가 세 전략의 exact trial/review/comparison과 새 DSR/PBO artifact ID·`collecting|diagnostic_ready` 상태를 같은 terminal artifact에 결속한다. 단일·두 전략은 `not_applicable`이고 legacy schema v2는 새 필드 없이 무재작성 검증된다. actual-shaped CLI first/replay에서 terminal/comparison/diagnostics artifact `1/1/1`, mode `600`, 두 상태 `collecting/collecting`, 외부 mutation 0건을 확인했다. 7월 23일 실제 strict session의 최초 결손은 02:44 KST 단일 KIS 분봉 read의 연속 `500`이었고, 실패 cycle은 보존했다. 이 원인은 오늘 exact `581eebc` forward runtime의 bounded 3회 server recovery와 closing-bar 재조회에 이미 반영돼 있다. exact `56cc4a0` schema v3 query-only audit은 기존 chain 뒤 7월 25일 06:50 및 7월 28일 06:25 KST에 별도 예약했다. 전체 `3591 passed`, Ruff, basedpyright `0/0`을 통과했다. 이는 아직 clean session이나 실제 DSR/PBO 성공 증거가 아니다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m8-terminal-overfit-audit-ko.md)에 있다.

**2026-07-24 M8 actual outcome-trace executor 업데이트:** 기존 7월 24·27일 research frozen SHA가 outcome-trace schema v2 도입 전임을 확인했다. actual run plan schema v3가 `required_outcome_trace_schema_version=2`를 plan ID에 결속하고 planned CLI가 exact `2`를 필수로 요구하며, loop는 artifact schema 불일치를 Reviewer 전에 차단한다. legacy plan schema v2는 새 필드 없이 byte-exact replay한다. 실행 전 receipt·claim·plan·trial이 모두 0건인 pre-v2 research/audit job만 current exact `61c5f39` runtime의 네 job으로 교체했다. 7월 25일 05:18 research·06:50 terminal audit, 7월 28일 05:40 research·06:25 terminal audit이 strict closeout 뒤 자동 실행된다. 전체 `3591 passed`, Ruff, basedpyright `0/0`, CLI help/bad/happy/replay `0/2/0/0`을 통과했다. 이는 아직 clean session이나 READY·성과 증거가 아니다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m8-actual-outcome-schema-schedule-ko.md)에 있다.
...
**2026-07-24 M8 Paper broker/shadow 승격 증거 업데이트:** US Day terminal이 broker/local reconciliation을 관측하지 않은 broker/shadow equality로 복제하던 오류를 fail-closed로 닫았다. 별도 query-only CLI는 strict daily research lineage와 읽기 전후가 같은 execution SQLite SHA를 결속하고 complete entry, protective OCO 또는 acknowledged EOD close FILL을 exact recommendation/session/strategy shadow trade와 짝짓는다. 60 paired session·100 paired trade, 양쪽 20bp 편도비용 후 PF 1.15·평균 양수·거래일 bootstrap CI 하한 0 문턱 전에는 항상 `collecting`이다. 실제 2026-07-23 입력은 paired `0/0`, mode-600 artifact `81c1ebe9...bbb57`, 모든 권한 false로 닫혔다. 전체 `3608 passed`, Ruff, basedpyright `0/0`, CLI help/bad/happy `0/1/0`을 통과했고 exact `2baa433` frozen runtime을 2026-07-25·28 07:15 KST에 별도 예약했다. 이는 equality나 Paper champion 성공 증거가 아니다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m8-broker-shadow-promotion-evidence-ko.md)에 있다.
...
**2026-07-23 strict forward post-session closeout 업데이트:** 장전부터 장마감까지 장시간 실행되는 watch 하나만 post-session metrics·daily research·adaptive evaluation을 생성하던 단일-owner 결손을 닫았다. 새 local-only closeout은 ranking/watch/retry/candidate cycle의 기존 strict progress가 clean이고 동일 뉴욕 거래일 정규장 종료 뒤이며 post terminal이 아직 없을 때만 정상 watch의 장후 chain을 복구한다. 기존 post 실패행, watch 실패, cycle coverage 결손은 삭제·재시도하지 않고 차단하며 성공 terminal은 historical query-only replay만 허용한다. causal source와 필수 metrics artifact, 전 종목 390분 coverage까지 다시 검증한다. 구현 SHA `a945ba4116a5e04e6823c2bdb27a02c47015aa9e`를 2026-07-24 04:55 KST at-most-once closeout으로 예약했으며 이는 actual clean session 성공 증거가 아니다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-23-forward-post-session-strict-closeout-ko.md)에 있다.
...
**2026-07-23 launchd 단발 실행 계약 업데이트:** `launchctl submit`으로 등록한 정상 종료 wrapper가 자기 label을 제거하지 않으면 daemon으로 반복 기동되는 실제 KR finalizer 사고를 확인했다. 새 `run_launchd_one_shot.py`는 timezone-aware 실행시각, 절대 실행파일·artifact 경로를 검증하고 mode-700 wrapper, mode-600 로그, atomic claim과 완료 receipt를 만든다. payload 종료 뒤 label을 제거하며 완료·진행 claim 재사용은 새 예약 전에 차단한다. 실제 임시 launchd label은 payload 1회, receipt exit 0, 종료 뒤 label 부재를 보였다. 이후 actual early progress가 제한된 launchd `PATH`에서 `#!/usr/bin/env ... uv`를 찾지 못해 exit `127`이 된 사건을 재현해, env shebang command는 예약 artifact를 만들기 전에 차단하고 `/bin/zsh` 또는 명시적 `uv` interpreter를 요구하도록 강화했다. 기존 KR finalizer와 Hermes는 변경하지 않았다. 상세 근거는 [실시간 검증 체크포인트](docs/checkpoints/2026-07-23-realtime-validation-schedule-ko.md)에 있다.

**2026-07-23 US forward 장중 진행 감사 업데이트:** final metrics 전에도 regular ranking coverage·watch·KIS retry/recovery·candidate input·Paper recommendation store의 strict cardinality와 failure를 함께 검사하는 local-only CLI를 추가했다. recovered retry는 관찰 정보로 남기되 unrecovered·repeated retry, 실패와 최소 cycle 미달은 그대로 차단하며 성공도 최종 자격이 아닌 `pending_post_session`이다. exact commit runtime으로 09:45 EDT 최소 8 cycle 조기 감사와 15:30 EDT 최소 300 cycle 후반 감사를 at-most-once launchd job으로 실제 등록했다. pre-open의 별도 premarket cycle/request `12/72`, failure `0/0`은 정규장 artifact와 섞지 않았다. 상세 근거는 [실시간 검증 체크포인트](docs/checkpoints/2026-07-23-realtime-validation-schedule-ko.md)에 있다.

**2026-07-23 US forward 장전 strict readiness 업데이트:** 장전 watch, NAS/NYS/AMS × updown/volume coverage, raw ranking snapshot row count, risk-selected candidate identity와 최신성을 네 mode-600 artifact의 결합 SHA에 대사하는 read-only CLI를 추가했다. 실패 request·watch cycle, cycle/cardinality·identity 불일치, stale/wrong-session 입력과 최신 후보 결손은 보고서에 그대로 남기고 차단한다. 실제 20:00 KST 수동 감사는 cycle/request/snapshot/latest selected `36/216/13625/10`, failure `0`으로 `ready`였다. exact SHA `3d488137ce6d612ebea98dd0b862e1fe9843ef44`의 22:25 KST at-most-once 검증도 exit `0`, cycle/request/snapshot/latest selected `65/390/29184/10`, 최신성 `139초`, gate 완화·외부 mutation `0`으로 완료됐고 종료 뒤 label이 제거됐다. 이는 정규장 최종 적격성이 아니다. 상세 근거는 [실시간 검증 체크포인트](docs/checkpoints/2026-07-23-realtime-validation-schedule-ko.md)에 있다.
...
**2026-07-23 M8 정규장 handoff·provenance 분리 예약 업데이트:** 장전 collector의 sleep을 official 09:30 EDT open으로 상한 처리해 첫 정규장 scan 지연을 닫고, actual coordinator의 dataset producer SHA와 frozen strategy code version을 별도 필드로 분리했다. 초기 exact clean `f917ffe` runtime으로 2026-07-27 장전→정규장 watch, readiness, strict closeout과 연구 체인을 예약했고, cadence 수정 뒤 forward·progress는 `5ecad89`, strict closeout과 Reviewer 재검증 뒤 closeout·research·audit은 `bc40069` runtime으로 교체했다. 품질 gate와 실패 cycle은 유지했고 executable Paper champion 두 개 전이라 Paper arm과 Allocation Manager는 열지 않았다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-23-m8-open-handoff-provenance-schedule-ko.md)에 있다.

**2026-07-23 M8 actual research terminal audit 업데이트:** 저장된 v2 manifest 파일의 trailing newline까지 포함한 실제 SHA와 in-memory canonical JSON SHA가 달라 trial provenance가 exact persisted artifact에 결속되지 않던 결손을 수정했다. query-only audit은 immutable run plan, research receipt/report, catalog·CSV·dataset receipt, READY foundation 1~3개, persisted manifest, completed trial·walk-forward artifact와 독립 Reviewer를 전부 다시 읽는다. Reviewer evidence·decision·reason도 원본 experiment와 ledger에서 다시 계산하고 strategy·evaluator·동일 cost identity를 대사하므로 self-consistent하게 재해시한 위조 review도 차단한다. lifecycle·주문·allocation 권한은 모두 false다. 현재 exact clean `bc40069` runtime으로 2026-07-27 연구 작업과 16:50 EDT 후속 audit을 예약했다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-23-m8-open-handoff-provenance-schedule-ko.md)에 있다.

**2026-07-24 M8 actual research standalone CLI 복구 업데이트:** causal dataset, catalog, READY input binding, multi-strategy loop, actual/planned coordinator와 terminal audit의 PEP 723 isolated 실행에서 shared KIS model의 `httpx2` 의존성이 빠져 `--help`부터 실패하는 결손을 수정했다. 구조적 metadata 회귀와 실제 isolated `--help`·bad binding·완전 fixture QA를 추가했고, fixture는 exact CSV SHA→READY manifest→trial→독립 Reviewer `hold`까지 완료했다. 커밋은 `5e6c4fd`, `b67a4f9`, `172b048`이며 전체 `3561 passed`, Ruff, basedpyright `0/0`을 통과했다. 기존 frozen 예약 payload와 품질 gate, 실패 cycle, Paper·allocation 권한은 바꾸지 않았다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m8-strict-actual-research-schedule-ko.md)에 있다.

**2026-07-23 US forward cycle cadence 업데이트:** 실제 정규장 첫 두 watch 시작 간격이 `76.7초`였고 이후에도 scan 소요시간이 고정 60초 sleep 위에 누적돼 1분 cadence를 지키지 못했다. `run_cycles`가 monotonic clock으로 operation 시간을 차감해 cycle 시작 간격을 유지하도록 수정했다. 17초 operation fixture는 43초만 대기했고 CLI 격리 happy path는 2-cycle 시작 간격 `1.003초`를 보였다. 당일 기존 frozen watch는 재시작하지 않았고 12-cycle strict progress는 ranking/watch/retry/candidate `12/12/12/12`, retry 회복 `36/36`, 반복 실패 `0`으로 clean이었다. exact `5ecad89` runtime으로 2026-07-27 forward와 early/late progress를 예약했으며 downstream closeout·research·audit은 아래 strict admission SHA를 사용한다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-23-m8-open-handoff-provenance-schedule-ko.md)에 있다.

**2026-07-23 M8 strict closeout→actual research admission 업데이트:** 2026-07-27 closeout 예약이 최소 watch cycle을 `1`로 설정했고 downstream actual research가 성공 receipt와 일반 strict marker만 검사해 이 완화 결과를 받아들일 수 있던 결손을 닫았다. strict closeout CLI 자체도 이제 기본값과 허용 하한이 모두 `300`이며 `299` 이하는 session mutation 전에 차단한다. actual research는 closeout의 ranking/watch/retry/candidate 네 cardinality가 동일한 `300~390` 범위일 때만 plan·dataset·trial 단계로 진입한다. 이를 포함한 exact `bc40069` runtime으로 closeout·research·terminal audit을 다시 예약했으며 아직 clean 실제 세션 성공 증거는 아니다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-23-m8-open-handoff-provenance-schedule-ko.md)에 있다.
...
**2026-07-24 KIS EOD 마지막 봉 semantic-lag 복구 업데이트:** 2026-07-23 strict closeout은 최종 cycle `355/355/355/355`, watch/retry failure `1/1`을 보존해 exit `1`로 닫혔다. 별도로 EOD catch-up의 29종목 중 LMTL·CIFU는 HTTP retry가 `500→200`으로 복구됐지만 성공 payload가 15:58까지만 포함해 마지막 15:59 봉 gate에서 실패했다. 최초 전체 조회 뒤 이 exact semantic 누락 종목만 2초·5초에 batch 재조회하고, 최대 세 번 뒤에도 없으면 기존 실패를 유지하도록 `581eebc`에서 수정했다. 후속 05:16 research와 05:26 독립 audit도 각각 exit `1/1`로 닫혔고 plan·CSV/receipt·READY foundation/manifest·trial/review·audit JSON은 모두 0개였다. 전체 `3563 passed`, Ruff, basedpyright `0/0`을 통과했고, 아직 시작 전이던 2026-07-24 full-session job을 exact clean `581eebc` runtime으로 교체 등록했다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m8-eod-last-bar-recovery-rollover-ko.md)에 있다.

**2026-07-24 M8 strict actual-research 예약 업데이트:** 2026-07-24 downstream dataset/research 예약이 실제 payload 실행 전이지만 오래된 SHA와 계약을 사용해 최신 strict closeout prerequisite와 독립 terminal audit이 빠져 있음을 확인했다. stdout/stderr 0바이트, plan/dataset/report 부재를 확인한 두 stale 대기 job만 제거하고 exact clean `8ef5904` runtime으로 05:06 KST strict closeout, 05:18 causal dataset·READY foundation·3전략 walk-forward·Reviewer, 05:35 exact persisted-manifest terminal audit을 at-most-once 연결했다. dataset producer SHA와 frozen strategy code version은 별도로 결속하며, 실패·지연은 부분 CSV나 trial로 바꾸지 않는다. 기존 frozen job은 그대로 두고 exact clean `91ff5d2` runtime의 schema v2 comparison 재감사를 7월 25일 06:35 및 7월 28일 06:10 KST에 추가 등록했다. 두 job은 이전 research artifact를 query-only로 재검증하고 별도 output root에만 발행하며, 성공 receipt나 exact plan·READY foundation·trial/review 결속이 없으면 그대로 차단한다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m8-strict-actual-research-schedule-ko.md)에 있다.
...
- [US systematic regime shadow vertical 체크포인트](docs/checkpoints/2026-07-22-us-systematic-regime-vertical-ko.md)
...
- [M5 issuer-direct announcement raw-first 체크포인트](docs/checkpoints/2026-07-23-m5-issuer-announcement-raw-first-ko.md)
- [OpenDART actual source 재검증 체크포인트](docs/checkpoints/2026-07-24-opendart-actual-source-revalidation-ko.md)
- [M6 Alpaca 옵션체인 raw-first 체크포인트](docs/checkpoints/2026-07-23-m6-alpaca-option-chain-raw-first-ko.md)
...
- [M6 actual option-chain bar 복구 체크포인트](docs/checkpoints/2026-07-23-m6-option-chain-actual-bar-recovery-ko.md)
- [M6 source-backed option skew 체크포인트](docs/checkpoints/2026-07-24-m6-source-backed-option-skew-ko.md)
- [M6 actual multi-expiration term structure 체크포인트](docs/checkpoints/2026-07-24-m6-option-term-structure-actual-ko.md)
- [M6 futures roll security master 체크포인트](docs/checkpoints/2026-07-24-m6-futures-roll-security-master-ko.md)
...
- [M6 ALFRED multi-vintage revision panel 체크포인트](docs/checkpoints/2026-07-24-m6-alfred-revision-panel-ko.md)
- [2026-07-24 KR M3 장후 검증 체크포인트](docs/checkpoints/2026-07-24-kr-m3-post-session-verification-ko.md)
...
- [M8 intraday equal-risk comparison evidence 체크포인트](docs/checkpoints/2026-07-24-m8-equal-risk-comparison-evidence-ko.md)
...
- [M8 Paper broker/shadow 승격 증거 체크포인트](docs/checkpoints/2026-07-24-m8-broker-shadow-promotion-evidence-ko.md)
...
- [US news-catalyst 일일 single-writer scheduler 체크포인트](docs/checkpoints/2026-07-21-us-news-catalyst-day-session-scheduler-ko.md)
...
```bash
uv run python run_us_swing_historical_replay.py \
  --fixture 2026-07-17=examples/us_swing_historical_replay/2026-07-17 \
  --fixture 2026-07-20=examples/us_swing_historical_replay/2026-07-20 \
  --experiment-ledger /tmp/us-swing-replay-experiment.sqlite3 \
  --shadow-ledger /tmp/us-swing-replay-shadow.sqlite3 \
  --delivery-store /tmp/us-swing-replay-delivery.sqlite3 \
  --review-ledger /tmp/us-swing-replay-review.sqlite3 \
...
├── run_sec_edgar_history_collect.py
├── run_sec_edgar_capability_registry.py
├── run_kis_kr_ranking_collect.py
├── run_kr_volume_surge_derive.py
├── run_kr_same_cycle_opportunity.py
...
- SEC EDGAR submissions의 recent·manifest-bound bounded additional-history, bounded issuer capability registry와 accession·primary-document-bound filing body raw-first vertical은 구현됐다. Alpaca licensed News도 bounded symbol/window raw-first 수집, local capability projection, 선언된 최대 400 symbols의 as-of coverage와 complete-only Opportunity 입력 evidence, 사전등록된 provider-time news-catalyst shadow baseline, 일별 treatment/zero-news control trial, current-session setup observation, frozen cohort 전체 전용 GET-only feature collection, 일일 single-writer scheduler와 독립 Reviewer까지 구현됐다. issuer-direct announcement는 권리·retention을 고정한 private onboarding, raw-first RSS/Atom 수집, exact replay와 bounded issuer capability까지 구현됐다. 실제 bounded production News GET 한 건은 raw-first 수집부터 10/10 coverage evidence와 shadow baseline `no_candidates` terminal까지 통과했다. 이는 시장 전체 coverage나 상시 운영을 뜻하지 않는다. WebSocket real-time news entitlement, 실제 권리가 검증된 production issuer onboarding·GET, launchd 상시 tick과 장기 actual forward 표본은 아직 없다. 실제 연락처 User-Agent를 임의 생성하지 않아 production SEC GET은 수행하지 않았다.
- bounded Alpaca option-chain GET은 단일 underlying·만기·right의 raw response와 latest quote/trade·IV·Greeks·minute/daily/previous-daily bar snapshot을 보존한다. actual AAPL call 두 만기의 contract master·indicative chain을 exact identity `77/77`, `78/78`로 결합하고 최대 300초 as-of 경계의 OI·all-strike median-IV term structure와 network-free replay까지 확인했다. 실제 term structure에서 near/far median IV 차이를 분류하는 content-addressed shadow context도 발행했다. READY call/put surface와 exact SIP completed-minute spot receipt를 요구하는 matched-strike·absolute-delta bucket skew도 구현했지만 actual SIP GET은 `403`으로 차단돼 실제 skew 표본은 0건이다. provider-neutral futures roll security master와 as-of resolver는 fixture로 구현했고, 공식 KIS 해외선물 GET-only quote source도 추가했지만 actual ES는 `EGW00550/CME SUB 미신청`으로 canonical quote 0건이다. actual CFTC TFF GET은 ES market-level weekly aggregate 두 시점의 다섯 category positioning context와 network-free replay를 확인했고, 두 immutable input과 reviewed binding을 결합하는 as-of positioning join은 actual CFTC에 fixture roll master를 결합한 증거까지 통과했다. 공식 Treasury actual GET은 14개 CMT maturity의 최신 두 일별 곡선과 network-free replay를 보존한다. KIS CME SUB entitlement 또는 별도 licensed CME·ICE source, 현재 계약 coverage, futures basis·curve, CFTC 장기 수정 이력·다중 market coverage와 derivatives performance trial·독립 Reviewer는 아직 없다. 이 제한된 slice를 실시간 파생상품 전체 수집이나 전략 성과로 해석하면 안 된다.
...
- US swing 신고가·RVOL v1은 완료 일봉 historical replay에서 현재시점 인과성, WATCH/NO_RECOMMENDATION 카드, 다중 세션 shadow terminal, global experiment trial과 독립 Reviewer까지 연결됐다. fixture E2E만 검증됐으며 current NYSE day-close production data, long-horizon forward 표본과 Paper 승격은 아직 없다.
