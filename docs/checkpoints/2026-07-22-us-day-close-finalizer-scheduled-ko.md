...
10:28 EDT 점검에서 finalizer 명령 뒤 종료 코드를 zsh 예약 변수 `status`에
대입하는 결함을 발견했다. 명령 실행 뒤 wrapper가 실패해 launchd가 동일
finalization을 다시 시작할 수 있으므로, 실행 전에 변수명을 `exit_code`로 바꾸고
해당 finalizer label만 재기동했다.

- finalizer 시각, source, strategy version과 fail-closed 조건은 바꾸지 않았다.
- 수정 뒤 `zsh -n`과 dry-run은 exit 0이다.
- 재기동한 one-shot finalizer는 장마감 시각까지 running 상태를 유지한다.
- arm 소비, Paper POST/DELETE와 broker order event는 0건이다.
