...
launchd label `ai.trading-agent.kr-m3-20260723`가 PID `41435`로 대기한다. exact detached runtime은 `/private/tmp/trading-agent-kr-m3-20260723-960de7e`이며 dirty main 작업트리를 실행 코드로 사용하지 않는다.
...
현재 Mac에는 mode-600 KIS와 LS 설정은 있지만 OpenDART 설정 파일은 없다. 09:05 전에도 이 상태가 유지되면 source cycle은 incomplete incident를 Hermes delivery outbox에 남기고 Opportunity, onboarding, 국내 계좌·주문 없이 nonzero로 종료한다. source가 완성된 경우에만 다음 단계가 열린다.
