# 스물여덟 번째 Alpaca Paper smoke 시도 체크포인트

작성 시각: 2026-09-03 09:45 EDT / 2026-09-03 22:45 KST
판정: **SAFE SKIP / STAGE 1 BLOCKED**

## 결론

`feature/paper-account-activities`의 local/remote 최신 `6ae9de9`에서 단계 1을 스물여덟 번째로 시도했다. 사용자 지정 기준 `7b033f3`은 이 commit의 ancestor다.

뉴욕 현지 시각은 09:41~09:45 EDT 정규장 시간대였고 GET-only bootstrap과 preflight는 기존 account binding, open order 0, position 0으로 통과했다. 그러나 주문 WSS readiness와 한 번의 targeted GET/WSS-only mutation recovery가 모두 `PaperOrderStreamUnavailableError`로 종료됐다. WSS 선행 실패로 broker clock은 관측하지 못했고, 첫 nonzero 게이트에서 후보 감사와 모든 armed CLI를 중단했다.

## 검증과 최종 상태

- 전체 회귀 946개, Ruff, basedpyright 0 errors/warnings를 통과했다.
- CLI help 4종, invalid arm 3종의 무생성 계약, fake broker 전 수명주기 E2E와 mutation 경계 표적 회귀 15개를 통과했다.
...
- broker CLI 최대 RSS는 58,212,352 bytes, 전체 검증 최대 RSS는 650,248,192 bytes였고 각 프로세스 page swap은 0이었다.
