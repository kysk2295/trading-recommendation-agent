...
- 개별 거래 bootstrap이 같은 날 거래를 독립 표본으로 취급하던 결함을 재현했다. 평가기 v2는 모든 종료 시각을 뉴욕 거래일로 정규화하고 날짜 전체를 블록으로 재표본화한다.
- 거래일 블록이 하나뿐인 표본은 퇴화된 CI를 표시하지 않고 `N/A`로 유지한다.
- 구형 거래단위 평가기 v1 원장과 v2 원장은 누적 적격일·거래 수를 합치지 않는다.
- 실제 CLI QA는 help 0, 존재하지 않는 입력 2, metrics 0, 일일 원장 0이었고 한국어 보고서와 원장에 날짜 블록·v2 계보가 남았다. 다른 승격 blocker가 남아 `promotion=false`를 유지했다.
- 전체 회귀는 수정 후 432개가 통과했다.
