...
- `run_kr_theme_projection.py`는 `--database`와 SQLite writer lock, journal, shared-memory, WAL sidecar를 정규화해 projection output 아래의 `opportunities.v1.jsonl`, `kr_theme_projection_summary_ko.md`와 비교한다.
- SQLite ledger 또는 sidecar가 두 산출물 중 하나와 겹치면 `KrThemeStore`를 열거나 classification을 append하기 전에 안전한 CLI 오류로 종료한다. 대상 산출물이 symlink인 경우도 닫힌다.
- KR projection JSONL outbox와 한국어 요약은 owner-only mode `600`으로 기록한다. replay에서 기존 outbox/report 권한이 변경됐더라도 다시 `600`으로 복구한다.
- 공용 `contract_outbox.py`는 변경하지 않았다. 이 경계는 KR projection CLI에만 적용된다.

## 검증

- focused projection/outbox 회귀: `uv run pytest -q tests/test_kr_theme_projection_cli.py tests/test_contract_outbox.py` -> `32 passed`
- 전체 자동 게이트: `uv run pytest -q` -> `1582 passed`
