...
- 한 adapter instance는 한 NYSE 세션의 한 종목만 처리한다. 휴장·정규장 밖·다중 desired subscription·quote/trade 계약 불일치는 HTTP 전에 닫힌다.
...
- 같은 분의 exact retry는 기존 raw page와 verified projection을 재사용해 `no_new_data`로 끝난다. provider minute 누락은 raw·canonical evidence 보존 뒤 supervisor의 `blocked_sequence_gap` incident가 된다.
...
- focused provider bridge: **8 passed**
- M4/raw/canonical 관련 회귀: **186 passed**
- full repository: **2167 passed**
- Ruff: 통과
