...
- 전체 HTTP 작업은 wall-clock 45초에 강제 중단하며 wire body와 압축 해제 body는 각각 64 MiB로 제한하고 원문 wire bytes와 content encoding을 파싱 전에 mode-600 append-only SQLite에 확정한다. raw 상한은 built-in HTTP·fixture, typed response와 store public boundary에서 실제 buffer 크기로 적용하고 exact `bytes`로 canonicalize한다.
