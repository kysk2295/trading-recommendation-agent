...
`run_kr_theme_day_session.py`는 장시간 sleep하는 daemon이 아니다. `init`이 한 거래 session의 immutable manifest를 mode 600으로 한 번 만들고, `launchd` 같은 scheduler가 `tick`을 반복 호출한다. 각 tick은 현재 KST와 exact calendar evidence 아래 지금 필요한 child만 별도 process로 직렬 실행한 뒤 종료한다.

manifest는 다음 identity를 SHA-256으로 함께 고정한다.

- strategy/code version, session date와 사전등록 시각
- official calendar snapshot, Opportunity와 KR symbol
...
canonical bytes, 현재 사용자 소유 regular file, mode 600과 single hard link가 아니면 tick 전에 차단한다. report와 audit에는 symbol, 가격, ID, 경로, raw payload와 credential을 쓰지 않는다.
...
- focused manifest/audit/supervisor/CLI: `11 passed`
- related KR child/session: `29 passed`
- 전체 회귀: `2751 passed`
- Ruff: 통과
- basedpyright: `0 errors, 0 warnings`
- compileall, 신규 production no-excuse: 통과
- actual CLI root/init/tick help와 missing manifest: exit `0/1`, blocked report mode `600`
