# 스물두 번째 Alpaca Paper smoke 시도 체크포인트

작성 시각: 2026-08-24 10:03 EDT / 2026-08-24 23:03 KST
판정: **SAFE SKIP / STAGE 1 BLOCKED**

## 결론

`feature/paper-account-activities`의 local/remote 최신 `ab0a77e`에서 단계 1을 스물두 번째로 시도했다. 사용자 지정 기준 `7b033f3`은 이 commit의 ancestor다.

production 로컬 NYSE 캘린더는 10:01 EDT를 열린 정규장으로 판정했고 GET-only bootstrap과 preflight는 기존 account binding, open order 0, position 0으로 통과했다. 그러나 주문 WSS readiness와 한 번의 targeted GET/WSS-only mutation recovery가 모두 `PaperOrderStreamUnavailableError`로 종료됐다. WSS 선행 실패로 broker clock은 관측하지 못했고, 첫 nonzero 게이트에서 후보 감사와 모든 armed CLI를 중단했다.
...
- broker CLI 최대 RSS는 58,179,584 bytes, 전체 검증 최대 RSS는 640,860,160 bytes였다.
