...
독립 manual QA에서 M7 실제 CLI happy path에 사용할 커밋 fixture가 없다는 사용성
차단도 확인했다. `examples/us_systematic_regime/2026-07-22`에 6개 ETF별 201세션,
총 1,206개 완료 일봉과 content hash manifest를 추가했다. 이 합성 자료는 CLI와
signal-only lifecycle 검증 전용이며 수익성 또는 실제 시장 coverage 증거가 아니다.

## 최종 통합 리뷰에서 수정한 결함

두 번째 독립 리뷰는 M5 완료 shortcut이 날짜와 terminal 증거만 확인하고 요청의
`runtime_code_version`과 fixture source hash를 확인하지 않는 문제를 차단했다. 같은
날짜의 과거 완료 증거가 다른 코드 또는 변경된 원본 데이터 요청에 재사용될 수 있었다.

완료 판정은 이제 experiment ledger에 저장된 정확한 strategy code version과 날짜별
root delivery에 저장된 `SwingDailySource.source_key`를 현재 요청에서 다시 계산한 값과
대조한다. 코드 또는 fixture가 달라지면 기존 증거를 변경하지 않고 fail-closed 한다.
동일 요청은 source identity를 검증한 뒤 scheduler를 다시 실행하지 않는다.

M6 lock-name 교체 회귀도 내부 contention이 아니라 lease body에서 이름을 교체한 뒤
context exit의 post-yield binding 검사가 직접 실패하는 형태로 강화했다.

- blocker RED: runtime code mismatch 1건 + fixture content mismatch 1건
- 수정 뒤 M5/M6 집중 회귀: 13 passed
- 변경 파일 Ruff: passed
- 변경 파일 basedpyright: 0 errors, 0 warnings, 0 notes

## 최종 통합 검증

깨끗한 통합 checkout에서 실제 공개 CLI 경계를 다시 실행했다.

- M5 help/invalid/happy/exact replay: `0/2/0/0`
- M5 replay evidence: delivery `3`, signal `1`, review `1`, trial `1`
- M6 help/invalid/happy/exact replay: `0/2/0/0`
- M6 replay evidence: trial `3`, experiment artifact `3`, review artifact `3`
- M7 help/invalid/committed-fixture happy/exact replay: `0/2/0/0`
- M7 replay evidence: signal-only card `1`, shadow trial `1`, account/order authority `0`
- private M5/M6/M7 result artifact mode: `0600`
- 전체 pytest: `3359 passed in 188.09s`
- pytest maximum RSS: `436,748,288 bytes`
- Ruff: passed
- basedpyright: `0 errors, 0 warnings, 0 notes`
- compileall과 `git diff --check`: passed
- credential, provider network, broker account/order, Paper POST/DELETE: `0`

최종 코드 리뷰는 unsafe hard-link lock을 setup에서 거부할 때 자체
`IntradayResearchLoopError`가 초기 cleanup handler를 우회해 시도당 parent와 lock
descriptor 두 개를 남기는 문제를 재현했다. 20회 반복 회귀는 수정 전 open FD가
`12 → 52`로 증가했다. setup 예외 경계가 해당 오류도 정규화하며 두 descriptor를
닫도록 수정했고, 같은 20회 반복에서 FD 수가 증가하지 않음을 고정했다.

- FD leak RED: 1 failed, open FD `+40`
- 수정 뒤 M6 집중 회귀: 9 passed
- 전체 pytest: `3359 passed`
- Ruff: passed
- basedpyright: `0 errors, 0 warnings, 0 notes`
