...
- worker launch 직전에 clean snapshot/root를 다시 검증하고, post-worker Git inventory 전에 repository symlink/`.git` topology와 repo-owned non-symlink `.git/index`(effective `git rev-parse --git-path index`)를 재검증한다.
...
현재 real in-place Grok worker 실행과 후속 review correction·hardening·final review blocker close가 반영됐다. harness는 `structuredOutput`만 소비하고, main-only preflight, process-group timeout kill, Git database fingerprint(shallow/grafts 포함), pre-existing index/sparse masking 거절, ambient GIT_* sanitize, launch/post-worker revalidation, user-owned/ignored metadata 보호, offline verification re-run, summary contract-safe enum을 강제한다.
...
- full repository pytest baseline at final-blocker close: **2105 passed**

## Release checklist (harness)

1. Contract paths/commands bounds and protected roots still reject closed.
2. Preflight rejects dirty checkout (except `?? .hermes/` / `?? .omo/`), non-main, linked worktree, symlink roots, pre-existing assume-unchanged/skip-worktree, sparse masking, and symlinked/non-owned `.git/index`.
3. Every ambient `GIT_*` env key is stripped for harness Git, worker, and verification subprocesses.
4. Launch revalidates clean snapshot/root; post-worker revalidates symlink/`.git` topology and index ownership before Git inventory.
...
6. Snapshot fingerprints refs/objects/control paths including `.git/shallow` and `.git/info/grafts`.
7. Production modules stay ≤250 pure LOC (no-excuse); focused harness tests + Ruff + basedpyright + CLI help pass; full suite baseline **2105**.
