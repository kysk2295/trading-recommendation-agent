# 열아홉 번째 Alpaca Paper smoke 시도 체크포인트

작성 시각: 2026-08-19 09:43 EDT / 2026-08-19 22:43 KST
판정: **SAFE SKIP / STAGE 1 BLOCKED**

## 결론

`feature/paper-account-activities`의 local/remote 최신 `6404286`에서 단계 1을 열아홉 번째로 시도했다. 사용자 지정 기준 `7b033f3`은 이 commit의 ancestor다.

production 로컬 NYSE 캘린더는 09:39 EDT를 열린 정규장으로 판정했고 GET-only bootstrap과 preflight는 기존 account binding, open order 0, position 0으로 통과했다. 그러나 주문 WSS readiness와 한 번의 targeted GET/WSS-only mutation recovery가 모두 `PaperOrderStreamUnavailableError`로 종료됐다. WSS 선행 실패로 broker clock은 관측하지 못했고, 첫 nonzero 게이트에서 후보 감사와 모든 armed CLI를 중단했다.

## 검증과 최종 상태

- 전체 회귀 946개, Ruff, basedpyright 0 errors/warnings를 통과했다.
- production mutation 경계 11개 파일의 Ruff format, CLI help 4종, invalid arm 3종의 무생성 계약과 fake broker 전 수명주기 E2E를 통과했다.
- 정적 금지패턴 감사에서는 기존 arm 입력 파서의 의도된 `value: object` 한 곳만 확인했고, `Any`, `cast`, type-ignore, `noqa`, broad `Exception`은 없었다. 코드 변경은 없었다.
...
- broker CLI 최대 RSS는 57,753,600 bytes, 전체 검증 최대 RSS는 638,353,408 bytes, 측정 swap은 0이었다.
