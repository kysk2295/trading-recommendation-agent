...
`/private/tmp/trading-agent-review-audit-20260727-bc40069`의 exact commit
`bc400690febe0fb376b68594290a20ea55764b34`로 교체했다. forward, closeout,
...
| 2026-07-28 05:20 KST / 2026-07-27 16:20 EDT | `ai.trading-agent.forward-post-session-20260727` | watch terminal을 최대 16:35 EDT까지 기다린 뒤 최소 300개 동일-cardinality strict local closeout | 20813 |
| 2026-07-28 05:40 KST / 2026-07-27 16:40 EDT | `ai.trading-agent.post-closeout-research-20260727` | strict 300-cycle closeout receipt/report를 요구한 뒤 actual causal dataset, READY foundation, multi-strategy walk-forward와 독립 Reviewer 실행 | 20819 |
| 2026-07-28 05:50 KST / 2026-07-27 16:50 EDT | `ai.trading-agent.actual-research-terminal-audit-20260727` | research receipt를 최대 17:05 EDT까지 기다린 뒤 exact persisted manifest, 1~3 READY foundation, completed trials와 독립 Reviewer를 원본에서 재계산 | 20824 |

research run은 다음 identity를 동결한다.

- run key: `actual-2026-07-27`
- dataset producer:
  `bc400690febe0fb376b68594290a20ea55764b34`
...
receipt·claim이 없는 것을 확인한 뒤 재등록했다. 아래 CLI 경계 강화 뒤 현재 PID는
`20813/20819/20824`이다.
...
## strict closeout CLI 최소치 고정

downstream admission이 1-cycle report를 거부해도 public closeout CLI 자체의
기본값과 허용 하한은 여전히 `1`이었다. commit
`cccb799c8e85b104abbafa3dc0e1b2cd3a9bf1d5`에서 운영 CLI의 기본값과 최소치를
모두 `300`으로 고정했다. library 함수는 fixture와 개별 복구 단위 테스트를 위해
기존 범위를 유지하지만 public CLI의 `299` 이하는 closeout 호출과 report 생성 전에
차단된다.
...
- Ruff 전체: pass
- basedpyright 전체: `0 errors, 0 warnings, 0 notes`
- 전체 pytest: `3471 passed`
- 기존 Grok offline environment 테스트: 변경과 무관한 `5 failed`
- no-excuse 검사: pass
- CLI help default `300`, minimum `299` exit `2`·artifact `0`
- 300-cycle actual-shaped historical replay: exit `0`, 네 cycle
  `300/300/300/300`, report mode `600`

23:15 KST query-only live progress audit도 현재 7월 23일 session의
ranking/watch/retry/candidate cycle `39/39/39/39`, KIS retry/recovery
`115/115`, repeated failure `0`, `progress_clean`, 최종 적격성
`pending_post_session`을 보였다. 당시 TradeSignal과 alert는 각각 2건이었으며
provider·계좌·주문 mutation은 없었다.

## independent Reviewer terminal 재계산

기존 terminal audit은 review artifact의 canonical hash, trial ID와 experiment
artifact ID는 검사했지만 evidence·decision·reason을 원본 walk-forward에서 다시
계산하지 않았다. 따라서 같은 trial/experiment를 가리키면서 evidence만 바꿔 새
artifact ID로 self-consistent하게 재해시한 review가 terminal audit을 통과했다.

commit `bc400690febe0fb376b68594290a20ea55764b34`에서 Reviewer의 순수 평가를
publication과 분리해 research 실행과 query-only terminal audit이 같은 원본
experiment·ledger에서 독립적으로 다시 계산하도록 했다. audit은 추가로 experiment의
strategy, evaluator version과 manifest의 동일 side-cost identity를 대사한다.

검증 결과:

- 위조 review 실제 RED: terminal audit이 잘못 ready
- 수정 뒤 같은 위조 review: CLI exit `1`, blocked
- canonical review actual-shaped CLI: exit `0`, ready
- focused: `17 passed`
- Ruff 전체: pass
- basedpyright 전체: `0 errors, 0 warnings, 0 notes`
- 전체 pytest: `3472 passed`
- 기존 Grok offline environment 테스트: 변경과 무관한 `5 failed`
- no-excuse 검사: pass

closeout/research/audit 세 job만 receipt·claim 부재를 확인하고 exact runtime으로 다시
등록했다. 전체 일곱 job은 `state=running`, `runs=1`, 새 세 PID는
`20813/20819/20824`, payload·wrapper mode `700`, 빈 stdout/stderr mode `600`,
receipt·claim 없음이었다. 현재 watch PID `62716`과 Hermes PID `31663`은 변경하지
않았다.
