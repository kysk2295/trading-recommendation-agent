...
`volume_surge`는 provider 응답이 아니므로 가짜 HTTP receipt를 만들지 않는다. terminal run의 `receipt_ids`는 빈 tuple이고 derived catalyst는 직접 observation으로 append한다. `KrThemeStore` source-run validator는 `VOLUME_SURGE`에만 이 형태를 허용한다. `NEWS`, `DART`, `KIS_RANKING`은 기존대로 모든 source observation이 run receipt에 연결돼야 하며 receipt 없는 provider run은 계속 거부한다. derived v2 payload의 `source_run_id`와 row별 `source_catalyst_id`가 upstream lineage를 담당한다.
...
- receipt 없는 derived `volume_surge` run 허용과 provider source의 기존 raw-receipt 강제 회귀
