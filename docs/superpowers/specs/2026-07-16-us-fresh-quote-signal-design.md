...
The parser retains the former v2 `invalid_quote` value only so an append-only outbox containing an already-written row remains readable. Producers canonicalize the same condition to `provider_failed`, and batch validation rejects `invalid_quote` as a new terminal append.
