...
`source_record_id`는 `collection_cycle_id + ranking_kind + symbol`의 cycle-local identity다. 같은 kind에서 종목 중복, rank 중복, malformed number, 빈 종목명과 6자리 아닌 symbol은 전체 source run을 실패시킨다. 다른 kind에 같은 종목이 있는 것은 정상이며 별도 catalyst로 보존한다.
...
production mode는 fixture가 없을 때 자동 선택되며 KIS live 시세 origin과 기존 mode-600 `~/.config/trading-agent/kis.env`만 사용한다. production의 `collection_date`는 실행 시점 KST 날짜와 정확히 같아야 하며, 이 검사는 DB 생성과 credential load보다 먼저 수행된다. fixture mode는 committed historical synthetic 응답을 허용하고 credential과 network를 사용하지 않는다.
...
이 collector가 검증된 뒤 별도 milestone에서 `kis_ranking`의 저장된 거래량 행만 읽어 canonical `KrVolumeSurgePayload` 하나를 만들고 `volume_surge` terminal source run을 확정한다. 두 source를 한 run으로 합치거나 현재 랭킹을 사후 재조회하지 않는다.
