...
| G003 / M1 | US·KR 추천·무추천·incident·일일요약의 Hermes/Telegram 전달 | **활성 다음 gate / 미통과** | Hermes adapter/worker는 존재한다. `outputs/hermes/delivery.sqlite3`: 42 events, 47 attempts, 31 ACKs; `watch 32 / incident 5 / no_recommendation 3 / daily_summary 2`. 다만 5-session top-level manifest가 없다 | `outputs/acceptance/hermes/manifest.json`의 US·KR 각 5개 실제 세션 reconciliation, restart/provider-fault report, 중복·누락 0 |
...
- Hermes 원장 기준은 `outputs/hermes/delivery.sqlite3`와 그 read-only acceptance receipts다. 현재 수치는
  전달 adapter의 존재와 일부 투영·ACK만 증명하며, 5-session top-level acceptance를 증명하지 않는다.
