...
- 사전등록: exact Opportunity Manager와 Day Agent strategy version, 고정 rank-1→VWAP 결합 규칙으로 cross-lane composite hypothesis를 content-address한다. 두 component authority와 code-coupled version을 확인한 뒤 전역 ledger에 먼저 append하며, day trial evidence budget은 hypothesis ID·registration key·Opportunity producer version을 고정한다.
...
- 재시작·검증: receipt 뒤 manifest write crash는 exact replay로 복구한다. tick과 독립 verifier는 receipt에서 같은 요청을 복원해 no-write onboarding replay를 먼저 요구하므로 legacy manifest, source drift와 다른 producer를 provider/child 전에 차단한다. production CLI에는 time override, account, order, arm, endpoint와 credential 옵션이 없다.
- 결과: 2026-07-20 synthetic same-cycle fixture의 DART→LS→KIS ranking→volume surge→Opportunity→onboarding→실제 supervisor 첫 tick E2E가 phase 5개, raw receipt 3건, shadow entry 1건과 audit 5건을 만들었다. 관련 112개와 전체 2777 tests, Ruff, basedpyright 0/0, changed-file format, compileall, JSON, no-excuse와 actual CLI QA가 통과했다. 실제 provider network와 국내 account/order mutation은 0건이며 열린 KRX read-only smoke는 후속 운영 검증이다.
