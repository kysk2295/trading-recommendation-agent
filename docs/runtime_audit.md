...
- durable evidence: session/date/minute, 장중 세 event ID와 세 attestation ID를 content-addressed schema v1 JSON으로 mode 600·single-link immutable publish한다. event·attestation content-address를 public boundary에서 재검증하고 최초 생성은 deterministic private pending artifact를 게시한 뒤 같은 stores에서 evidence를 다시 계산해 일치할 때만 final 경로를 게시한다. final은 현재 pending inode의 no-overwrite hard link로 승격한다. alias publisher가 link 성공을 직접 관측한 뒤 실패한 경우에만 내부 cleanup하고, caller는 publish 예외의 소유권을 추측하지 않는다. 성공 반환 뒤 source/artifact 검증 또는 report 실패 cleanup도 device/inode·size·mtime·ctime·link count 전체가 일치하는 no-follow parent dirfd entry에만 한정한다. 운영 CLI clock만 사용하고 시각 override를 받지 않는다. exact replay는 evidence 재게시 없이 publication lock과 `chmod`/`fchmod`가 없는 evidence·manifest·receipt·Opportunity query projection을 사용한다. ledger 조회는 private source descriptor와 parent identity에 결합된 단일 in-memory SQLite snapshot을 onboarding과 source-state 모두에서 사용하고 non-mode-700 parent·symlink·hard-link·main/WAL/SHM drift를 차단한다. report는 no-follow parent descriptor에 staging과 replace를 고정하고 path identity를 전후 대사한다. 저장된 장중 시각으로 현재 stores를 다시 검증하므로 폐장 뒤 재시작도 provider 없이 가능하며 source drift는 차단된다.
...
- 결과: production-shaped first/replay, fixture·장외·register/start 누락·불완전 phase·future EOD/history·event/cycle 시각 mismatch·다른 cycle의 더 최신 sequence·future source·검증 및 final publish 중 source drift·post-link failure·pending unlink 뒤 parent fsync/state-refresh failure rollback·rollback unlink failure의 invalid two-link 보존·same-inode relink와 parent-swap foreign-final preservation·post-publish와 success-report failure cleanup·lock/chmod-free onboarding/evidence replay·ledger hard-link/symlink/WAL drift·report directory swap·symlink-loop report alias·forged content-address·casefold/pending/report/evidence/derived-store path alias·protected-file containment·artifact tamper와 missing-manifest CLI를 집중 58개, 관련 282개 및 전체 2846개로 검증했다. 전체 pytest는 148.75초에 통과했고, 2026-07-20 10시대 KST에는 당일 장 전 등록된 production manifest가 작업공간에 없어 actual smoke를 차단했다. 실제 provider GET, production open-smoke evidence와 국내 account/order mutation은 0건이다.
