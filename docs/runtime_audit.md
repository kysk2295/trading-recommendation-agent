...
- 사전등록: exact Opportunity Manager와 Day Agent strategy version, 고정 rank-1→VWAP 결합 규칙으로 cross-lane composite hypothesis를 content-address한다. 두 component authority와 code-coupled version을 확인한 뒤 전역 ledger에 먼저 append하며, day trial evidence budget은 hypothesis ID·registration key·Opportunity producer version을 고정한다. 최초 composite·trial CLI append는 입력 등록시각이 실제 현재시각보다 0~5분 이내이고 KST 09:00 전일 때만 허용하며 exact replay만 이후 재실행할 수 있다.
- onboarding: 장중 CLI는 exact trial ID, Opportunity ID와 private store 경로만 받는다. trial/composite/calendar, Opportunity producer·same-session freshness·validity, 하나뿐인 collection cycle과 rank-1 symbol을 query-only 재생해 identity를 유도한다. immutable receipt를 manifest보다 먼저 fsync하고 두 파일의 owner/mode 600/single-link/canonical bytes를 검증한다. root부터 no-follow directory descriptor를 유지해 staging fsync와 no-overwrite hard-link 게시를 수행한다. final link 뒤 중단으로 남은 staging alias는 exact payload·inode replay만 single-link로 복구하며, reader도 연 descriptor에서 metadata와 bytes를 읽어 ancestor·leaf path swap을 차단한다.
