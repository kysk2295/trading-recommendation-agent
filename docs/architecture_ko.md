...
KR 장후 control cycle은 terminal finalizer, independent Reviewer와 Lifecycle Controller를 세 child process로 분리한다. parent runner는 local store path와 immutable identity만 전달하고 credential, provider, endpoint, timestamp override, account/order option을 전달하지 않는다. 각 child exit 뒤 전용 mode-600 CSV audit append가 성공해야 다음 child를 시작하며 terminal 실패는 Reviewer와 Controller를, Reviewer 실패는 Controller를 시작하지 않는다. terminal exact replay는 이미 확정된 sequence 2의 최초 `occurred_at`을 재사용해 같은 session의 늦은 재시작이 새 artifact identity를 만들지 않는다. aggregate completed는 연구 control cycle 완료일 뿐 전략 수익이나 승격 판정이 아니다.
