...
- 첫 smoke의 `--liquidity-quantity`는 1로 고정한다. 1주가 100 USD 한도를 넘으면 주문은 차단되어야 한다.
...
## 4. exact current ORB 후보 고정

다음 query는 추천, 추천 생성 당시 후보 입력, 최신 완료 1분봉의 최초 관찰을 같은 SQLite에서 결합한다. 현재 30초 이내에 생성됐고 현재 시계 기준 직전 완료 1분봉을 가리키는 `setup` ORB 후보만 CSV에 남긴다.

```bash
mkdir -p "$SMOKE_RUN/05_candidate"
sqlite3 -header -csv "$WATCH_RUN/paper_recommendations.sqlite3" > "$SMOKE_RUN/05_candidate/candidate.csv" <<'SQL'
WITH linked AS (
  SELECT
    r.recommendation_id,
    r.symbol,
...
`candidate.csv`는 header와 data 1행만 있어야 하고 `eligible_count`는 1이어야 한다. 0건 또는 2건 이상이면 그 cycle에는 entry를 실행하지 않는다. CSV의 값을 반올림·현재시각으로 대체하지 않는다. `recommendation_id`를 그대로 `intent-id`로 사용해 KIS 추천과 Alpaca `client_order_id` 계보를 유지한다.

## 5. armed entry 1회

아래 placeholder 전부를 `candidate.csv`의 같은 한 행으로 치환한다. 명령을 실행하기 전 현재 분이 바뀌었으면 새 candidate query부터 다시 시작한다.
...
./run_alpaca_paper_entry_smoke.py \
  ...
  --bar-start '<bar_start>' \
  --bar-first-observed '<bar_first_observed>' \
  --liquidity-quantity 1 \
...
부분 또는 전체 체결이 확인되면 같은 `recommendation_id`로 보호 OCO를 즉시 실행한다.
...
- 현재 분 rollover 뒤 stale bar 인자를 그대로 사용하려는 경우
