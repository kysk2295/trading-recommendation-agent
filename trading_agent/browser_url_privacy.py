...
def has_sensitive_browser_url_metadata(path: str, query: str) -> bool:
    return (
        contains_browser_secret_text(_fully_decoded_component(path))
        or contains_browser_secret_text(_fully_decoded_component(query))
        or any(
...
def _fully_decoded_component(value: str) -> str:
    decoded = value
    while True:
...
__all__ = ["has_sensitive_browser_url_metadata"]
