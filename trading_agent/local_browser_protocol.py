...
from trading_agent.browser_url_privacy import has_sensitive_browser_url_metadata
...
def _fragment_contains_credentials(fragment: str) -> bool:
    if fragment == "user:password":
        return True
    candidates = (fragment, fragment if fragment.startswith("//") else f"//{fragment}")
...
def require_public_https_url(value: str) -> str:
    ...
    if _fragment_contains_credentials(parsed.fragment) or has_sensitive_browser_url_metadata(parsed.path, parsed.query):
        raise _protocol_error()
