...
from trading_agent.research_identity_models import AgentOperatingMode, StrategyLaneRef

_OPPORTUNITY_HYPOTHESIS_ID = "H-KR-THEME-MOMENTUM-001"
_DAY_HYPOTHESIS_ID = "H-KR-THEME-LEADER-VWAP-001"


@dataclass(frozen=True, slots=True)
class _RegistrationContract:
    strategy_lane: StrategyLaneRef
    strategy_version_base: str


_CONTRACTS: Final = {
    _OPPORTUNITY_HYPOTHESIS_ID: _RegistrationContract(
        KR_THEME_OPPORTUNITY_LANE,
        "kr-theme-keyword-projection-v1",
    ),
    _DAY_HYPOTHESIS_ID: _RegistrationContract(
        KR_THEME_LEADER_VWAP_RECLAIM_LANE,
        "kr-theme-leader-vwap-reclaim-v1",
    ),
}
...
class KrThemeResearchRegistrationManifest(BaseModel):
    ...
    def validate_manifest(self) -> Self:
        ...
        if (
            ...
            or self.hypothesis_id not in _CONTRACTS
            or self.strategy_version != _strategy_version(self.hypothesis_id, self.code_version)
...
class KrThemeProjectionAuthorityRequest(BaseModel):
    ...
    def validate_request(self) -> Self:
        expected_versions = tuple(_strategy_version(key, self.code_version) for key in _CONTRACTS)
        if self.strategy_version not in expected_versions or not _aware(self.projected_at):
            raise InvalidKrThemeResearchRegistrationError
        return self
...
def kr_theme_strategy_version(code_version: str) -> str:
    return _strategy_version(_OPPORTUNITY_HYPOTHESIS_ID, code_version)


def kr_theme_day_strategy_version(code_version: str) -> str:
    return _strategy_version(_DAY_HYPOTHESIS_ID, code_version)


def _strategy_version(hypothesis_id: str, code_version: str) -> str:
    if not code_version or code_version != code_version.strip():
        raise InvalidKrThemeResearchRegistrationError
    contract = _CONTRACTS.get(hypothesis_id)
    if contract is None:
        raise InvalidKrThemeResearchRegistrationError
    digest = hashlib.sha256(code_version.encode()).hexdigest()[:16]
    return f"{contract.strategy_version_base}-code-{digest}"
...
def require_registered_kr_theme_strategy(
    ...
    return _require_registered_strategy(ledger, request, KR_THEME_OPPORTUNITY_LANE)


def require_registered_kr_theme_day_strategy(
    ledger: ExperimentLedgerReader,
    request: KrThemeProjectionAuthorityRequest,
) -> MultiMarketStrategyVersionRegistration:
    return _require_registered_strategy(ledger, request, KR_THEME_LEADER_VWAP_RECLAIM_LANE)


def _require_registered_strategy(
    ledger: ExperimentLedgerReader,
    request: KrThemeProjectionAuthorityRequest,
    expected_lane: StrategyLaneRef,
) -> MultiMarketStrategyVersionRegistration:
    matches = tuple(
        stored.registration
    ...
    if (
        ...
        or registration.strategy_lane != expected_lane
...
def kr_theme_research_registrations(
    ...
    scope = MultiMarketExperimentScope(
        ...
        primary_lane=contract.strategy_lane,
        lanes=(contract.strategy_lane,),
    ...
    version = MultiMarketStrategyVersionRegistration(
        ...
        hypothesis_id=manifest.hypothesis_id,
        experiment_scope_key=scope_key,
        strategy_lane=contract.strategy_lane,
