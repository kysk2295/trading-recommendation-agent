...
def quote_actionability_artifacts_match(
    ...
    expected_evidence = {
        ...
        for evidence in (
            *base_signal.evidence_refs,
            EvidenceRef(
                namespace="signal/conditional",
                record_id=base_signal.signal_id,
                observed_at=base_signal.observed_at,
            ),
            EvidenceRef(
    ...
    expected_evidence_refs = tuple(
        expected_evidence[key] for key in sorted(expected_evidence)
    )
    return (
        validation is not None
        and _base_is_current(
            base,
        ...
        and signal.producer_strategy_version
        == base_signal.producer_strategy_version
        ...
        and signal.evidence_refs == expected_evidence_refs
    )
...
def quote_actionability_assessment_matches(
    ...
    if not _in_regular_session(assessment.evaluated_at):
        return (
    ...
    if (
        snapshot.symbol != base.signal.symbol
        or assessment.quote_id != snapshot.quote_id
    ):
        return False

    expected_status = _snapshot_terminal_status(
        base,
        snapshot,
        evaluated_at=assessment.evaluated_at,
    )
    ...
    if expected_status in {
        ...
        return derived is not None and quote_actionability_artifacts_match(
            base,
            snapshot,
            assessment,
            derived,
