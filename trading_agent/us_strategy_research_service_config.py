...
US_STRATEGY_RESEARCH_SERVICE_LABEL: Final = "ai.trading-agent.us-strategy-research-source"
...
class UsStrategyResearchServiceConfig(BaseModel):
...
class UsStrategyResearchLaunchAgentVerification:
    ready: bool
...
def write_us_strategy_research_service_config(
    path: Path,
    ...
    try:
        ...
        raise InvalidUsStrategyResearchServiceError(reason="service_config_write_invalid") from None


def load_us_strategy_research_service_config(path: Path) -> UsStrategyResearchServiceConfig:
    try:
        payload = read_private_text(path.expanduser().absolute())
        config = UsStrategyResearchServiceConfig.model_validate_json(payload)
        if payload != _config_text(config):
            raise InvalidUsStrategyResearchServiceError(reason="service_config_noncanonical")
        return config
    except (
        InvalidPrivateImmutableFileError,
        InvalidUsStrategyResearchServiceError,
        ...
        raise InvalidUsStrategyResearchServiceError(reason="service_config_read_invalid") from None


def write_us_strategy_research_launch_agent(
    path: Path,
    config: UsStrategyResearchServiceConfig,
    config_path: Path,
) -> bool:
    try:
        ...
        raise InvalidUsStrategyResearchServiceError(reason="launch_agent_write_invalid") from None


def verify_us_strategy_research_launch_agent(
    config_path: Path,
    plist_path: Path,
) -> UsStrategyResearchLaunchAgentVerification:
    try:
        absolute_config = config_path.expanduser().absolute()
        config_text = read_private_text(absolute_config)
        config = load_us_strategy_research_service_config(absolute_config)
        plist_text = read_private_text(plist_path.expanduser().absolute())
        if plist_text != _launch_agent_text(config, absolute_config):
            raise InvalidUsStrategyResearchServiceError(reason="launch_agent_contract_invalid")
        required = (config.uv_path, config.project_root / _SERVICE_SCRIPT, config.project_root / _CYCLE_SCRIPT)
        if (
            not config.project_root.is_dir()
            or any(not path.is_file() for path in required)
            or not os.access(config.uv_path, os.X_OK)
        ):
            raise InvalidUsStrategyResearchServiceError(reason="service_executable_binding_invalid")
        return UsStrategyResearchLaunchAgentVerification(
            ready=True,
            interval_seconds=US_STRATEGY_RESEARCH_SERVICE_INTERVAL_SECONDS,
        ...
        raise InvalidUsStrategyResearchServiceError(reason="launch_agent_verify_invalid") from None


def _config_text(config: UsStrategyResearchServiceConfig) -> str:
    return (
        json.dumps(
...
def _launch_agent_text(config: UsStrategyResearchServiceConfig, config_path: Path) -> str:
    payload = {
        "Label": config.label,
        "ProgramArguments": [
            str(config.uv_path),
        ...
        "RunAtLoad": True,
        "StartInterval": US_STRATEGY_RESEARCH_SERVICE_INTERVAL_SECONDS,
        "ThrottleInterval": 30,
