...
from trading_agent.bls_public_schema import (
    BLS_PUBLIC_SCHEMA_VERSION,
    CREATE_BLS_PUBLIC_SCHEMA,
)
...
class BlsPublicStoreError(ValueError):
    @override
    def __str__(self) -> str:
        return "BLS public data store is invalid"


@final
class BlsPublicStore:
    __slots__ = ("path",)
        ...
        try:
            with closing(self._connection(write=True)) as connection:
                ...
                if existing is not None:
                    ...
                    return False
                connection.execute(
                    "INSERT INTO bls_public_receipts VALUES (?,?,?,?,?,?,?,?)",
                    row,
                )
                connection.commit()
            return True
        except (OSError, sqlite3.Error, TypeError, ValueError):
            raise BlsPublicStoreError from None

    def append_run(self, run: BlsPublicRun) -> bool:
        ...
        try:
            with closing(self._connection(write=True)) as connection:
                connection.execute("BEGIN IMMEDIATE")
                receipt = self._receipt_row(connection, run.request.request_id)
                ...
                if existing is not None:
                    ...
                    return False
                connection.execute(
                    "INSERT INTO bls_public_runs VALUES (?,?,?,?)",
                    row,
                )
                connection.commit()
            return True
    ...
    def receipt(self, request_id: str) -> BlsPublicRawResponse | None:
        ...
        try:
            ...
            if row is None:
                return None
            request = BlsPublicRequest.model_validate_json(row[1])
            response = BlsPublicRawResponse(
                request_id=row[0],
    ...
    def run(self, request_id: str) -> BlsPublicRun | None:
        if not self.path.exists():
            return None
        try:
            with closing(self._connection(write=False)) as connection:
            ...
            if row is None:
                return None
            run = BlsPublicRun.model_validate_json(row[2])
            payload = canonical_experiment_ledger_json(run).encode()
            if (
                ...
                or row[2] != payload
                or self._stored_receipt_id(request_id) != run.receipt_id
            ):
                raise BlsPublicStoreError
            return run
        except (OSError, sqlite3.Error, TypeError, ValueError):
    ...
    def counts(self) -> tuple[int, int]:
        try:
            with closing(self._connection(write=False)) as connection:
                receipts = connection.execute("SELECT COUNT(*) FROM bls_public_receipts").fetchone()
                runs = connection.execute("SELECT COUNT(*) FROM bls_public_runs").fetchone()
    ...
    def _stored_receipt_id(self, request_id: str) -> str | None:
        with closing(self._connection(write=False)) as connection:
            row = self._receipt_row(connection, request_id)
    ...
    def _receipt_row(
        ...
        return connection.execute(
            ...
            "FROM bls_public_receipts WHERE request_id=?",
            (request_id,),
        ).fetchone()
    ...
    def _run_row(
        ...
        return connection.execute(
            ...
            "FROM bls_public_runs WHERE request_id=?",
            (request_id,),
        ).fetchone()
        ...
        try:
            ...
            if write:
                ...
                if connection.execute("PRAGMA user_version").fetchone() == (0,):
                    connection.executescript(CREATE_BLS_PUBLIC_SCHEMA)
                    connection.execute(f"PRAGMA user_version={BLS_PUBLIC_SCHEMA_VERSION}")
                    connection.commit()
            else:
                ...
                try:
                    ...
                    os.close(parent)
            if connection.execute("PRAGMA user_version").fetchone() != (BLS_PUBLIC_SCHEMA_VERSION,):
                connection.close()
                raise BlsPublicStoreError
            return connection
...
__all__ = ("BlsPublicStore", "BlsPublicStoreError")
