...
def experiment_ledger_v7_identity(path: Path) -> str:
    reader = ExperimentLedgerReader(path)
    if not reader.is_initialized():
        raise ValueError("experiment ledger v7 authority is invalid")
    connection = sqlite3.connect(f"file:{reader.path}?mode=ro", uri=True)
    try:
        _ = connection.execute("PRAGMA query_only = ON")
        version = connection.execute("PRAGMA user_version").fetchone()
        if version != (7,):
            raise ValueError("experiment ledger schema is not v7")
...
__all__ = ("experiment_ledger_v7_identity",)
