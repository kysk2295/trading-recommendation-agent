...
class DayAgentVersionWriter:
    __slots__ = ("_connection", "_opened", "_path")

    def __init__(self, path: Path) -> None:
        self._path = path
        self._connection: sqlite3.Connection | None = None
        self._opened: OpenVersionStore | None = None

    def __enter__(self) -> Self:
        opened = open_version_store_writer(self._path)
        try:
            connection = sqlite3.connect(self._path, timeout=0.0)
            ...
            close_version_store_writer(self._path, opened)
            raise DayAgentVersionStoreError("version_store_open_failed") from error
    ...
    def __exit__(
        ...
        if self._opened is not None:
            close_version_store_writer(self._path, self._opened)
            self._opened = None
    ...
    def register_challenger(self, version: AgentVersion) -> bool:
        ...
        try:
            ...
            raise DayAgentVersionStoreError("version_store_write_failed") from error
