...
def test_materializer_binds_exact_producer_commit_to_receipt(tmp_path: Path) -> None:
    # Given: a strict source session and the exact clean materializer commit.
    source = tmp_path / "source"
    write_closed_source_session(source, include_censored_symbol=False)

    # When: the source is materialized into historical research input.
    ...
    # Then: the canonical content-addressed receipt binds the producer commit.
...
def test_dataset_cli_exposes_sessions_and_runs_happy_path(tmp_path: Path) -> None:
    ...
    assert "--producer-commit-sha" in help_result.stdout
