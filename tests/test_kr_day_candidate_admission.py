...
def _policy(**updates: Decimal | int | str) -> KrDayCandidateAdmissionPolicy:
    values: dict[str, Decimal | int | str] = {
        ...
        "min_completed_bar_price_response": Decimal("0.005"),
        "max_spread_bps": Decimal("20"),
    }
    values.update(updates)
    return KrDayCandidateAdmissionPolicy.model_validate(values)


def _bars(*, low_volume: bool = False, neutral: bool = False) -> tuple[KrCompletedMinuteBar, ...]:
    close = "100" if neutral else "103"
    volume = 100 if low_volume or neutral else 200
    return (_bar(0, close="100", volume=100), _bar(1, close=close, volume=volume))


def _bar(index: int, *, close: str, volume: int, end_offset: int | None = None) -> KrCompletedMinuteBar:
