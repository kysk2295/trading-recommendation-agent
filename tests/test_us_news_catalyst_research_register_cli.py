...
import run_us_news_catalyst_research_register as registration_cli
from trading_agent.experiment_ledger_store import ExperimentLedgerStore

PROJECT = Path(__file__).resolve().parents[1]
SCRIPT = PROJECT / "run_us_news_catalyst_research_register.py"
MANIFEST = PROJECT / "examples" / "us_news_catalyst" / "research-registration.json"
REPORT_NAME = "us_news_catalyst_research_registration_ko.md"


def test_us_news_catalyst_research_register_direct_help_is_self_contained() -> None:
    ...
    assert "news-catalyst" in completed.stdout


def test_us_news_catalyst_research_register_fixture_replays_privately(
    tmp_path: Path,
) -> None:
...
def test_us_news_catalyst_research_register_missing_manifest_blocks_cleanly(
    tmp_path: Path,
) -> None:
