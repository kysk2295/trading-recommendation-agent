...
- live actionability lifecycle CLI는 manifest/active plan/fresh policy state를 대사하고 explicit read-only arm·정규장 뒤 bounded dynamic quote/trade epoch를 raw-first 수집한다. terminal이 original snapshot과 같은 completed-minute일 때만 terminal 시각으로 READY feature를 재관측해 projector를 실행한다. restart는 connector 0건 exact replay이며 minute rollover, quote-only, public credential은 output 0이다. 아직 runtime supervisor가 이 CLI를 자동 child-dispatch하지는 않는다.
...
1. runtime fleet supervisor가 새 manifest를 만든 같은 cycle 안에서 live actionability lifecycle을 optional single-writer child로 dispatch하고, child success/replay/blocked를 supervisor 원장에 결합한다. arm·receipt root·actionability store·credential 옵션은 모두 있어야 활성화하고 partial 설정은 provider/state 전에 차단한다.
2. 열린 NYSE 정규장과 mode-600 Alpaca market-data credential·SIP entitlement가 자연스럽게 동시에 맞을 때만 live actionability CLI로 bounded read-only smoke를 실행한다. exact control 3개·quote/trade data receipt·terminal·reobserved snapshot·actionability append를 대사하고 계좌·주문·Paper endpoint는 열지 않는다. 휴장에는 fixture E2E 결과만 유지한다.
