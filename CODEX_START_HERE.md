...
- actual 2026-07-23 Alpaca SIP stream smoke는 인증 control에서 공식 provider code 409 `insufficient_subscription`으로 차단됐다. exact `00af3d3` query-only admission은 이 persisted attempt를 content-addressed `blocked` artifact로 만들었고 input DB SHA·mtime과 artifact 수를 replay에서 유지했다. bounded-complete만 `ready`, 402·406·transport·protocol failure는 `unknown`이며 이 증거가 있는 동안 동일 SIP 조건을 반복 성공으로 가장하지 않는다.
