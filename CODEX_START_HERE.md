...
- actionability SQLite schema v2는 신규 artifact와 exact runtime manifest ID·snapshot 관측시각의 content-addressed creation row를 한 transaction에 append한다. v1 query는 무재작성이고 다음 v2 Writer만 migration하며 legacy artifact를 사후 backfill하지 않는다. live projector와 verifier의 v2 연결은 아직 남아 있다.
...
2. live projector가 manifest-aware schema v2 append를 사용하게 하고 cross-store verifier가 creation row를 우선 대사하도록 연결한다. legacy v1 artifact는 생성 시도를 추정하지 않고 기존 보수적 terminal-time 검증만 유지한다.
