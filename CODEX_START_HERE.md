...
2. 실제 read-only stream smoke에서 disconnect/reconnect 또는 provider rejection이 관측되면 구현된 epoch history·failed attempt와 private evidence를 대사한다. bounded supervisor를 추가하기 전에는 자동 재연결이나 무제한 retry를 수행하지 않는다.
