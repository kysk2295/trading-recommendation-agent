...
- exact `61c5f39` actual run plan schema v3는 required outcome-trace schema `2`를 plan ID에 결속하고 artifact 불일치를 Reviewer 전에 차단한다. legacy plan v2는 byte-exact replay한다. pre-v2 frozen research/audit 예약은 실행 전 current-code schema-v2 research와 terminal schema-v3 audit 네 job으로 교체됐으며 strict closeout 실패 시 CSV·trial을 만들지 않는다.
