...
1. durable active dynamic plan을 query-only로 읽는 별도 stream owner CLI를 연결한다. 첫 runtime cycle이 plan을 append한 뒤 bounded read-only WebSocket이 그 plan의 raw receipt와 terminal을 저장하고, 다음 minute manifest projector가 같은 plan의 snapshot 시점 이전 receipt만 소비하도록 fixture E2E를 먼저 완성한다.
2. 열린 NYSE 정규장과 mode-600 Alpaca market-data credential·SIP entitlement가 자연스럽게 동시에 맞을 때만 active plan stream owner로 단일 plan/frame read-only smoke를 실행한다. exact control 3개·data receipt·terminal을 대사하고 계좌·주문·Paper endpoint는 열지 않는다. 휴장에는 fixture E2E 결과만 유지한다.
