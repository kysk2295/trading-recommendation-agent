...
1. 열린 NYSE 정규장과 mode-600 Alpaca market-data credential·SIP entitlement가 자연스럽게 동시에 맞을 때만 `run_alpaca_sip_trade_stream_smoke.py`로 단일 종목 frame 1개 read-only smoke를 실행한다. exact control 3개·data receipt·canonical replay·terminal attestation과 private report를 대사하고 계좌·주문·Paper endpoint는 열지 않는다. 휴장에는 fixture E2E 결과만 유지한다.
2. 실제 read-only stream smoke에서 disconnect/reconnect 또는 provider rejection이 관측되면 구현된 epoch history·failed attempt와 supervisor lifecycle audit을 대사한다. production smoke에 bounded supervisor를 연결하기 전 fixture의 3회 budget보다 넓히지 않고 OS signal 기반 종료·heartbeat 증거를 장중 soak에서 확인한다.
