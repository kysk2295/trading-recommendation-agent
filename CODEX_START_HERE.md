...
- M4.2 READY quote+trade desired set은 한 fresh Alpaca SIP connection용 deterministic multi-symbol request plan으로 결합된다. exact ACK는 trade·quote·automatic correction/cancel-error symbol 집합과 빈 extra channel을 검증하며 provider list 순서는 강제하지 않는다. 아직 raw-first multi-symbol connection/data store나 실제 WebSocket 운영 결합은 열지 않았다.
