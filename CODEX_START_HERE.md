...
2. 실제 read-only stream smoke에서 disconnect/reconnect가 관측되면 구현된 epoch history와 private evidence를 대사한다. provider pre-auth error·connection-limit·handshake failure도 자격증명이나 원문 오류를 남기지 않는 terminal attempt evidence로 보존한다.
