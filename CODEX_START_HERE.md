...
- query-only live evidence verifier는 completed child를 exact-minute manifest와 원래 source manifest identity, digest receipt의 bounded terminal, plan/epoch와 actionability bundle까지 대사한다. created receipt 누락·public mode·child split mismatch는 aggregate-only blocked report로 닫는다. 이전 crash terminal을 현재 artifact new append와 구분할 durable attempt binding은 아직 없어 해당 모호성은 fail-closed다.
...
2. 실제 장중 smoke 뒤 supervisor child aggregate를 manifest별 raw receipt terminal과 actionability envelope에 대사하는 별도 cross-store verifier를 추가한다. child 실패를 fleet minute-bar 성공으로 덮지 않고 이미 저장된 raw receipt도 삭제하지 않는다.
