...
- bounded reconnect supervisor가 위 결정을 owner invocation에 연결한다. timeout·socket·handshake failure만 persisted budget 안에서 즉시 재시도하고 endpoint/protocol/ACK failure는 현재 failed terminal을 보존한 채 다음 connector 전에 차단한다. 기존 complete·budget exhaustion은 connector 0건이며 local receipt integrity 오류는 report로 숨기지 않는다. backoff와 실제 provider smoke는 아직 남아 있다.
