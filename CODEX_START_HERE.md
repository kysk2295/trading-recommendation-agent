...
- verified dynamic replay의 첫 3개 control을 다시 검증하고 data receipt의 quote·trade·correction·cancel wire message를 exact plan instrument에 귀속하는 immutable projection을 추가했다. unbound symbol, future/wrong-date event와 control-in-data는 차단한다. 아직 correction chain 상태 전이, terminal attestation과 실제 provider smoke는 남아 있다.
