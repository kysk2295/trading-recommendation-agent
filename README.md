...
> **현재 상태:** 분봉 수집·급등주 스캐너·ORB/VWAP/HOD/Gap-and-Go 추천·인과성 감사·과거 및 forward 평가에 더해 Alpaca Paper 계좌·미체결 주문·포지션 GET, 단일 Writer 원장 bootstrap과 읽기 전용 preflight까지 구현되어 있다. 주문 제출·취소·청산 API는 아직 공개하지 않으므로 현재 실행 명령은 주문을 제출하지 않는다.
...
Alpaca paper 코드는 trading base URL을 설정값으로 자유롭게 받지 않는다. 정확한 `https://paper-api.alpaca.markets`만 허용하고 다른 URL은 네트워크 요청 전에 거절한다. 리다이렉트도 따르지 않는다. KIS는 계속 시세 조회 전용이다.
...
- Alpaca Paper 계좌·미체결 주문·포지션 GET 대사와 단일 Writer 원장 결합은 구현됐다. 실제 주문 제출, trade update stream, 부분체결 보호주문, kill switch와 마감 전 강제청산은 아직 구현되지 않았다.
