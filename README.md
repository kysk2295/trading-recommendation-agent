...
- SEC EDGAR submissions의 recent·manifest-bound bounded additional-history, bounded issuer capability registry와 accession·primary-document-bound filing body raw-first vertical은 구현됐다. Alpaca licensed News도 bounded symbol/window raw-first 수집과 local capability projection까지 구현됐지만 production GET, WebSocket real-time entitlement, issuer-direct announcement onboarding과 여러 issuer를 묶는 시장 단위 coverage assessment는 아직 없다. 실제 연락처 User-Agent를 임의 생성하지 않아 production SEC GET은 수행하지 않았다.
