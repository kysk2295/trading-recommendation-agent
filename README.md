...
**2026-07-24 KIS EOD 마지막 봉 semantic-lag 복구 업데이트:** 2026-07-23 strict closeout은 최종 cycle `355/355/355/355`, watch/retry failure `1/1`을 보존해 exit `1`로 닫혔다. 별도로 EOD catch-up의 29종목 중 LMTL·CIFU는 HTTP retry가 `500→200`으로 복구됐지만 성공 payload가 15:58까지만 포함해 마지막 15:59 봉 gate에서 실패했다. 최초 전체 조회 뒤 이 exact semantic 누락 종목만 2초·5초에 batch 재조회하고, 최대 세 번 뒤에도 없으면 기존 실패를 유지하도록 `581eebc`에서 수정했다. 전체 `3563 passed`, Ruff, basedpyright `0/0`을 통과했고, 아직 시작 전이던 2026-07-24 full-session job을 exact clean `581eebc` runtime으로 교체 등록했다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-24-m8-eod-last-bar-recovery-rollover-ko.md)에 있다.
