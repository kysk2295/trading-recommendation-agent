...
**2026-07-20 SEC EDGAR bounded additional-history 업데이트:** recent raw receipt의 `filings.files`를 파일명·CIK·건수·시작일·종료일이 고정된 typed manifest로 승격하고, exact `https://data.sec.gov/submissions/{declared-file-name}` GET만 허용하는 별도 CLI를 추가했다. 한 invocation은 recent GET과 합쳐도 SEC 10 requests/second 경계 아래인 최대 8개 history 파일만 선언 순서대로 처리하며 첫 terminal failure에서 중단한다. 각 child collection ID는 부모 receipt와 exact manifest의 canonical hash로 결정되고, child run payload는 부모 receipt와 manifest를 보존한다. 기존 SQLite schema를 변경하지 않으면서 store는 부모 recent success run·raw receipt·manifest membership·CIK·child ID·provider receipt 시각·terminal 순서를 모든 append와 replay에서 다시 대사한다. history raw response는 파싱 전에 기존 mode-600 append-only receipt에 확정되고 top-level parallel filing column, manifest count/date 범위와 accession CIK가 모두 맞을 때만 기존 accession correction chain에 canonical observation을 추가한다. HTTP/구조 실패 raw, transport terminal, parse 이전 orphan과 완료 terminal은 provider·fixture·User-Agent를 다시 열지 않고 복구한다. 실제 PEP 723 CLI에서 recent fixture `2/2`, history fixture `1/1`, missing User-Agent provider-free replay `1/0`을 확인했다. focused SEC **111 tests**, 전체 **2964 tests**, Ruff와 basedpyright 0/0가 통과했으며 실제 SEC production GET과 broker/account/order mutation은 0건이다.
