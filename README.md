...
> **현재 상태:** 분봉 수집·급등주 스캐너·ORB/VWAP/HOD/Gap-and-Go 추천·인과성 감사·과거 및 forward 평가 코드는 구현되어 있다. 실제 Alpaca paper 주문 어댑터는 설계가 승인됐지만 아직 구현 전이다. 따라서 현재 실행 명령은 주문을 제출하지 않는다.
...
추천 유무와 관계없이 `kis_ranking_snapshots.csv`에는 관찰 시각, 랭킹 출처, 거래소, 원천 순위, 가격·등락률·호가·거래량·거래대금과 실제 선택 여부가 append-only로 저장된다. `market_risk_screen.csv`에는 공식 현재 거래정지, 호가 결손·역전, spread, 편도 20bp 슬리피지 예비비를 합친 예상 왕복비용과 최종 선정 여부를 별도로 누적한다.
...
`active` 뒤 손절·2R·당일 종료가 확인된 추천만 거래로 집계한다. 출력은 `paper_metrics.csv`, `paper_yearly_metrics.csv`, `paper_trades.csv`, `paper_metrics_ko.md`이며, 현재 저장된 2건은 기능 검증용 QA 표본이므로 수익성 증거가 아니다.
...
├── trading_agent/        추천·전략·리스크·KIS 연결 코드
...
자격증명은 프로젝트 안에 저장하지 않는다. 현재 읽기 전용 로컬 설정은 `~/.config/trading-agent/kis.env`와 `~/.config/trading-agent/alpaca.env`를 사용한다. paper 실행 기반을 구현할 때는 별도 `~/.config/trading-agent/alpaca-paper.env`만 사용한다. 토큰 캐시는 `~/.cache/trading-agent/`에 두며 모든 비밀 파일은 권한 `600`을 요구한다.

Alpaca paper 실행 코드는 trading base URL을 설정값으로 자유롭게 받지 않는다. 정확한 `https://paper-api.alpaca.markets`만 허용하고 다른 URL은 네트워크 요청 전에 거절한다. KIS는 계속 시세 조회 전용이다.
...
- Alpaca 실제 paper 주문, 계좌·포지션 대사, 부분체결 처리와 마감 전 강제청산은 승인된 설계와 구현 계획만 존재하며 아직 코드로 구현되지 않았다.
