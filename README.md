...
- SEC EDGAR submissions의 recent·manifest-bound bounded additional-history, bounded issuer capability registry와 accession·primary-document-bound filing body raw-first vertical은 구현됐다. Alpaca licensed News도 bounded symbol/window raw-first 수집, local capability projection, 선언된 최대 400 symbols의 as-of coverage와 complete-only Opportunity 입력 evidence, 사전등록된 provider-time news-catalyst shadow baseline까지 구현됐다. 이는 시장 전체 coverage가 아니며 production News GET, WebSocket real-time entitlement, issuer-direct announcement onboarding, baseline의 일별 forward trial·zero-news control·독립 Reviewer는 아직 없다. 실제 연락처 User-Agent를 임의 생성하지 않아 production SEC GET은 수행하지 않았다.
