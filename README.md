...
> **현재 상태:** 분봉 수집·급등주 스캐너·ORB/VWAP/HOD/Gap-and-Go 추천·인과성 감사·과거 및 forward 평가에 더해 Alpaca Paper 시장시계·계좌·미체결 주문·포지션 GET, `trade_updates` 스트림 인증·구독·Ping/Pong, 단일 Writer 원장과 fail-closed 주문 승인 게이트까지 구현되어 있다. 승인 게이트는 활성 스트림 안에서 REST와 원장을 대사하고 부분체결을 한 노출로 합산하며 비용 포함 수량을 내부 산정한다. 주문 제출·취소·청산 API는 아직 공개하지 않으므로 현재 실행 명령은 주문을 제출하지 않는다.
...
- Alpaca Paper GET 대사, 주문 스트림 control plane과 heartbeat, 활성 세션 기반 정규장/current-bar/portfolio 승인 게이트는 구현됐다. 기존 부분체결을 승인 전 위험에 합산할 수는 있지만 `trade_updates` 체결 이벤트 영속화, 실제 주문 제출, 부분체결 보호주문, kill switch와 마감 전 강제청산은 아직 구현되지 않았다.
