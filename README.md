...
- global experiment ledger schema v5는 기존 v1~v4 행을 유지하면서 exact `StrategyLaneRef` 기반 multi-market hypothesis, strategy version, shadow trial과 started/terminal event chain을 별도 append-only table에 보존한다. KR theme Opportunity Manager와 `theme_leader_vwap_reclaim` day agent의 code-coupled shadow version은 각각 exact lane으로 등록되고, day agent는 고정 evidence budget의 daily trial 등록/start까지 연결됐다. multi-market research source card, KR day shadow fill·terminal, lifecycle과 독립 Reviewer는 아직 남아 있다.
