...
**2026-07-19 multi-market shadow trial 업데이트:** global experiment ledger schema v5에 multi-market trial과 최대 두 event의 append-only chain을 추가했다. v1~v4 행은 재작성하지 않으며 current schema의 table/index/trigger exact-set도 Reader와 Writer가 검증한다. 첫 consumer인 KR `theme_leader_vwap_reclaim` day lane은 exact code-coupled shadow version, 공식 KIS 거래일 스냅샷으로 확인된 session의 09:00 KST 전 등록, 고정 no-entry baseline·20 sessions·30 signals·20bp 비용·결측 0·Reviewer gate를 요구한다. local CLI는 `register`와 `start`만 제공하며 fill, terminal, lifecycle, 계좌·주문 권한은 열지 않는다.
...
**2026-07-19 KR theme day restartable session supervisor 업데이트:** `run_kr_theme_day_session.py init/tick`은 content-addressed mode-600 session manifest 하나로 pre-open trial register, open start, minute별 KIS GET-only collect→shadow entry→exit, 15:30 EOD catch-up→exit와 장후 terminal→Reviewer→lifecycle child를 직렬 연결한다. tick은 오래 sleep하지 않고 scheduler가 반복 호출하는 단발 process이며, append-only phase audit를 재생해 성공한 같은 cycle은 건너뛰고 실패 child부터 재시도한다. collector 뒤에는 현재시각을 다시 읽어 receipt보다 과거인 평가시각을 만들지 않으며 같은 장중 minute를 넘으면 fail-closed한다. 실제 subprocess intraday entry/replay와 no-entry 일일 censored/review/lifecycle fixture E2E, 전체 **2751 tests**, Ruff, basedpyright 0/0, compileall, no-excuse가 통과했다. 실제 열린 KRX session KIS GET smoke와 launchd 배치는 아직 수행하지 않았고 국내 account/order mutation은 0건이다.
...
KR day shadow trial은 day manifest를 먼저 등록한 뒤 장 전에 `register`, 장 시작 뒤 `start`를 실행한다. 아래 값은 committed fixture QA 전용이며 실제 forward run은 clean checkpoint SHA로 만든 day strategy version과 실제 다음 KRX session을 사용한다.
...
./run_kr_theme_day_trial.py start \
  --trial-id trial-kr-theme-vwap-20260720-d9c4bd25af15f188 \
  --occurred-at 2026-07-20T09:00:00+09:00 \
  --database outputs/experiment_control/experiment_ledger.sqlite3 \
  --output-dir outputs/experiment_control/kr-theme-day-trial/latest
```

운영에서는 위 child를 수동으로 이어 붙이지 않고 `./run_kr_theme_day_session.py init --help`로 session manifest를 한 번 만든 뒤 scheduler가 `tick --manifest ... --output-dir ...`를 반복 호출한다. manifest는 calendar snapshot, strategy/code version, Opportunity, symbol과 각 append-only store 경로를 content-addressed identity로 고정한다. production tick에는 현재시각 override가 없고 fixture manifest 두 경로는 all-or-none이다. 15:30 EOD raw가 없는 채 15:31 이후 재시작하거나 register/start source attestation이 없으면 장후 성공으로 복구하지 않고 차단한다. phase audit sibling의 `*-evidence.sqlite3`도 owner/mode 600/single-link append-only store이며 함께 보존해야 한다. 각 운영 tick 뒤에는 `./run_kr_theme_day_session_verify.py --manifest ... --output-dir ...`로 child/provider를 열지 않는 cross-store 대사를 실행할 수 있다.
...
- OpenDART 공시검색, LS NWS 뉴스 제목, KIS 국내 랭킹 source run, 저장된 KIS evidence에서 canonical `volume_surge` v2를 파생하는 상태기계와 네 terminal run을 확정하는 coordinator가 구현됐다. DART·LS normalized catalyst는 exact keyword rules replay를 거쳐 별도 research evidence artifact로 투영할 수 있다. `run_kr_same_cycle_collect.py`는 네 source를 같은 새 cycle ID·날짜로 직렬 실행하고 exact terminal replay에서는 provider를 열지 않는다. fixture E2E만 검증됐으며 bounded 동일-cycle production 수집, 그 evidence의 KR Opportunity·TradeSignal 연결과 기사 본문 `t3102`는 아직 수행하지 않았다.
...
- global experiment ledger schema v5는 기존 v1~v4 행을 유지하면서 exact `StrategyLaneRef` 기반 multi-market hypothesis, strategy version, shadow trial과 started/terminal event chain을 별도 append-only table에 보존한다. KR theme Opportunity Manager와 `theme_leader_vwap_reclaim` day agent의 code-coupled shadow version은 각각 exact lane으로 등록되고, day agent는 daily trial 등록/start, conservative entry·exit/PnL·terminal과 독립 multi-session Reviewer까지 연결됐다. multi-market research source card와 KR day lifecycle projection은 아직 남아 있다.
