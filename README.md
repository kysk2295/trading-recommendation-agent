...
- bounded Alpaca option-chain GET은 단일 underlying·만기·right의 raw response와 latest quote/trade·IV·Greeks·minute/daily/previous-daily bar snapshot을 보존하고 local derivatives capability로 투영한다. actual AAPL contract master 77개와 OI 68개, 정규장 indicative chain 77개를 query-only로 exact identity 결합한 `READY` surface와 network-free replay까지 확인했다. 시장 전체 옵션 coverage, OPRA entitlement, 다중 만기·strike의 IV·OI surface/skew/term structure, 선물 basis·curve·roll adapter와 derivatives 전략 agent는 아직 없다. 이 제한된 slice를 실시간 파생상품 전체 수집으로 해석하면 안 된다.
