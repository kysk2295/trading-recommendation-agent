...
- bounded Alpaca option-chain GET은 단일 underlying·만기·right의 raw response와 latest quote/trade·IV·Greeks snapshot을 보존하고 local derivatives capability로 투영한다. 별도 option-contract security master의 actual AAPL GET은 고유 계약 77개와 OI 68개를 materialize했지만, 예약된 정규장 chain smoke 결과, 시장 전체 옵션 coverage, OPRA entitlement, IV·OI term structure, 선물 basis·curve·roll adapter와 derivatives 전략 agent는 아직 없다. 두 제한된 adapter를 실시간 파생상품 전체 수집으로 해석하면 안 된다.
