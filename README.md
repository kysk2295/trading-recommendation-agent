...
이 CLI는 provider, 자격증명, network, LLM, 현재가와 주문 코드를 호출하지 않는다. production DART·LS 뉴스 제목·KIS 국내 랭킹 adapter와 DB-only canonical `volume_surge` 파생기는 각각 구현됐지만, 네 단계를 같은 새 cycle ID로 순서 실행하는 오케스트레이터와 동일-cycle production 검증은 아직 남아 있다. coverage CSV와 한국어 요약은 aggregate 감사 자료이며 추천 품질이나 수익성 증거가 아니다.
...
- OpenDART 공시검색, LS NWS 뉴스 제목, KIS 국내 랭킹 source run, 저장된 KIS evidence에서 canonical `volume_surge` v2를 파생하는 상태기계와 네 terminal run을 확정하는 coordinator는 구현됐다. LS adapter는 실제 OAuth·WebSocket bounded read-only QA까지 통과했고 KIS 국내 랭킹도 exact GET 2건의 bounded production read-only smoke와 no-network replay를 통과했다. 기존 production KIS 원장에 대한 local-only 파생도 거래량 30행·영문 포함 코드 7개 보존과 replay 신규 0행을 확인했다. 다만 네 source를 같은 새 cycle ID로 실행하는 오케스트레이터, 최종 동일-cycle production 검증과 기사 본문 `t3102`가 남아 있어 아직 새 production KR Opportunity을 확정하지 않는다.
