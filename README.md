...
- global experiment ledger schema v4는 기존 v3 US legacy hypothesis/version/authority 행을 유지하면서 exact `StrategyLaneRef` 기반 multi-market hypothesis와 strategy version을 별도 append-only table에 보존한다. KR theme Opportunity Manager와 `theme_leader_vwap_reclaim` day agent의 code-coupled shadow version은 각각 exact lane으로 등록·재생·소비 gate에 연결됐다. multi-market research source card, append-only KR day trial/fill, lifecycle과 독립 Reviewer는 아직 남아 있다.
