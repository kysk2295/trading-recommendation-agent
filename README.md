...
아래 세 명령은 KR Opportunity Manager의 code-coupled shadow 가설·전략 버전을 전역 원장에 먼저 사전등록하고, 별도 synthetic cycle을 적재한 뒤 committed keyword rules와 저장된 `volume_surge` BLOB으로 theme Opportunity을 만든다. 전체 과정은 local-only이며 현재 호가, LLM, KIS/Alpaca 호출, TradeSignal과 주문을 사용하지 않는다.
...
- KR day shadow 앞의 provider-neutral market gate는 세션·VI·단일가·거래정지·투자지정, 상하한가·+27% 근접, 5초 quote freshness와 bid/ask usability를 explicit evidence로 요구한다. clear 상태만 `eligible`이며 unknown은 모두 차단한다. 아직 LS/KIS adapter, KRX 달력, KR indicator·TradeSignal·shadow fill은 연결하지 않았다.
...
- global experiment ledger schema v4는 기존 v3 US legacy hypothesis/version/authority 행을 유지하면서 exact `StrategyLaneRef` 기반 multi-market hypothesis와 strategy version을 별도 append-only table에 보존한다. KR theme Opportunity Manager의 code-coupled shadow version은 등록·재생·projection gate에 연결됐지만, multi-market research source card, trial, lifecycle, Reviewer와 KR day TradeSignal/shadow fill은 아직 남아 있다.
