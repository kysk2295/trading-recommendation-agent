...
- Alpaca Paper GET 대사, 주문 스트림 control plane·heartbeat, raw-first `trade_updates` 영속화·격리·재시작 복구, Account Activities FILL 기반 누락 execution 보강, 부분체결 보호 OCO 계획과 미보호 노출 admission 차단, 단일 WSS/Writer generation barrier는 구현됐다. Alpaca WSS에는 replay cursor가 없고 FILL API에는 별도 correction/bust 계약이 명시돼 있지 않아 모순은 계속 fail-closed 처리한다. OCO의 durable intent·nested leg 대사와 실제 제출·교체·취소, kill switch와 마감 전 강제청산은 아직 구현되지 않았다.
