...
- SEC EDGAR submissions의 recent·manifest-bound bounded additional-history raw-first vertical과 bounded issuer capability registry projection은 구현됐다. filing document 본문과 회사 발표 evidence, 여러 issuer를 묶는 시장 단위 coverage assessment는 아직 없다. 실제 연락처 User-Agent를 임의 생성하지 않아 production SEC GET은 수행하지 않았다.
