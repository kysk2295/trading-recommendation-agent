...
> **현재 상태:** 분봉 수집·급등주 스캐너·ORB/VWAP/HOD/Gap-and-Go 추천·인과성 감사·과거 및 forward 평가에 더해 Alpaca Paper 시장시계·계좌·주문·포지션·Account Activities FILL GET, `trade_updates` 스트림 인증·구독·Ping/Pong, 단일 Writer 원장과 fail-closed 주문 승인 게이트까지 구현되어 있다. 수신 frame은 text/binary 원문 BLOB을 먼저 확정한 뒤 분류하며, 재시작 시 미분류 receipt를 원래 순서로 복구하고 매 연결 세대에서 REST 주문 snapshot·개별 FILL activity와 대사한다. 새 통합 운영 세션은 한 Writer lease와 한 WSS 안에서 ingestion·current-epoch REST recovery·admission을 비동시로 실행하고, 복구 뒤 SQLite local/external generation이 바뀌면 승인을 폐기한다. 주문 제출·교체·취소·청산 API는 아직 공개하지 않으므로 현재 실행 명령은 주문을 변경하지 않는다.
...
- Alpaca Paper GET 대사, 주문 스트림 control plane·heartbeat, raw-first `trade_updates` 영속화·격리·재시작 복구, Account Activities FILL 기반 누락 execution 보강, 단일 WSS/Writer generation barrier와 활성 세션 기반 정규장/current-bar/portfolio 승인 게이트는 구현됐다. Alpaca WSS에는 replay cursor가 없고 FILL API에는 별도 correction/bust 계약이 명시돼 있지 않아 모순은 계속 fail-closed 처리한다. 실제 주문 제출, 부분체결 보호주문, kill switch와 마감 전 강제청산은 아직 구현되지 않았다.
