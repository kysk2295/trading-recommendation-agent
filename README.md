...
- bounded Alpaca option-chain GET은 단일 underlying·만기·right의 raw response와 latest quote/trade·IV·Greeks·minute/daily/previous-daily bar snapshot을 보존한다. actual AAPL call 두 만기의 contract master·indicative chain을 exact identity `77/77`, `78/78`로 결합하고 최대 300초 as-of 경계의 OI·all-strike median-IV term structure와 network-free replay까지 확인했다. READY call/put surface와 exact SIP completed-minute spot receipt를 요구하는 matched-strike·absolute-delta bucket skew도 구현했지만 actual SIP GET은 `403`으로 차단돼 실제 skew 표본은 0건이다. 시장 전체 옵션 coverage, OPRA entitlement, call/put parity, per-strike volatility surface, 선물 basis·curve·roll adapter와 derivatives 전략 agent는 아직 없다. 이 제한된 slice를 실시간 파생상품 전체 수집이나 전략 성과로 해석하면 안 된다.
