...
운영에서는 위 child를 수동으로 이어 붙이지 않고 `./run_kr_theme_day_session.py init --help`로 session manifest를 한 번 만든 뒤 scheduler가 `tick --manifest ... --output-dir ...`를 반복 호출한다. manifest는 calendar snapshot, strategy/code version, Opportunity, symbol과 각 append-only store 경로를 content-addressed identity로 고정한다. production tick에는 현재시각 override가 없고 fixture manifest 두 경로는 all-or-none이다. 15:30 EOD raw가 없는 채 15:31 이후 재시작하거나 register/start audit이 없으면 장후 성공으로 복구하지 않고 차단한다.
