...
- lane manifest·binding·experiment scope·daily snapshot registry, ORB intraday final snapshot producer, 독립 Reviewer event projection과 fail-closed 장후 순차 runner는 구현됐다. Reviewer는 권고만 저장하며 champion과 Portfolio Manager는 아직 없다. Portfolio Manager는 최소 두 lane champion 전에는 구현하거나 주문권한을 갖지 않는다.
