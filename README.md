...
**2026-07-18 US Always-On M4.0~M4.3 업데이트:** verified canonical replay identity, 완료된 연속 1분봉의 VWAP·ATR·RSI·MACD·RVOL·breakout 공통 kernel, broad scanner 후보를 bounded quote/trade desired set으로 바꾸는 순수 정책에 provider-neutral read-only runtime supervisor를 연결했다. supervisor는 별도 mode-600 SQLite와 비차단 single-writer lease 아래 raw receipt를 먼저 append하고 checkpoint·sequence gap·reconnect evidence를 append-only로 보존한다. 재시작은 마지막 sequence에서 이어지고 exact duplicate는 재발행하지 않으며, 충돌 duplicate와 gap은 fail-closed다. gap이 없는 완료 봉만 M4.1 kernel로 전달하고 stale/closed 정책은 adapter 호출 전에 막는다. fixture E2E에서 정상 수집·재시작·중복·gap·새 epoch 복구를 검증했으며 provider·network·credential·계좌·주문 경로는 아직 연결하지 않았다. 전체 **2151 tests**와 Ruff, basedpyright, compileall, no-excuse가 통과했고 다음 checkpoint는 evidence-gated US Opportunity projection M4.4다.
...
- Data Plane foundation과 US read-only supervisor의 fixture runtime은 구현됐지만 실제 US provider adapter, capability/entitlement registry 영속화와 health 수집, 전 종목 상시 raw stream, object-storage catalog 및 deterministic cross-source replay는 아직 없다. fixture receipt를 live entitlement나 운영 coverage로 해석하면 안 된다.
