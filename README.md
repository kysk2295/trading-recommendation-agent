...
**2026-07-18 US Always-On M4.0~M4.2 업데이트:** verified canonical replay를 bounded research scope에 결합하는 `ResearchInputIdentity`, 완료된 연속 1분봉에서 VWAP·ATR·RSI·MACD·RVOL·breakout을 계산하는 공통 feature kernel, broad scanner 후보를 bounded quote/trade desired set으로 바꾸는 순수 dynamic subscription policy를 구현했다. 구독 정책은 exact replay identity, 정책 semantic version과 전체 config를 decision에 보존하며 priority score·source rank·instrument ID 순으로 안정 정렬한다. hard capacity, incumbent minimum residency, deterministic eviction, unsubscribe-before-subscribe와 eviction cooldown을 적용하고 instrument-symbol 불일치·중복·future/naive timestamp를 fail-closed로 거부한다. stale snapshot이나 NYSE 정규장 밖 입력은 desired set을 비우며 scanner candidate 자체에는 구독 권한이 없다. 이 단계는 provider adapter·network·credential·계좌·주문을 import하거나 호출하지 않는다. M4.0~M4.2 집중 검증 54개와 전체 **2143 tests**가 통과했고, 다음 checkpoint는 read-only runtime supervisor M4.3이다.
