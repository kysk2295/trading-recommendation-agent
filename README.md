...
**2026-07-18 US Always-On M4.0~M4.4 업데이트:** verified canonical replay identity, 완료된 연속 1분봉의 공통 indicator kernel, bounded quote/trade 구독 정책과 restart 가능한 read-only supervisor를 기존 US Opportunity·conditional TradeSignal 경로까지 연결했다. M4 전용 evidence gate는 candidate마다 exact ready feature를 요구하고 missing·gap·stale·insufficient·noncausal evidence를 구조화된 사유로 차단한다. ready feature는 지표 값을 Opportunity나 Signal에 복사하지 않고 전체 snapshot을 결합한 SHA-256 evidence reference만 추가하며, base Opportunity 전체 내용·평가시각·evidence를 결합한 새 immutable Opportunity ID를 만든다. 이 gate를 통과한 경우에만 기존 publisher가 호출되고 signal은 계속 `conditional`, `quote_validation=None`이다. 기존 scanner와 기존 publication API는 변경하지 않았다. fixture E2E와 전체 **2159 tests**, Ruff, basedpyright, compileall, no-excuse가 통과했으며 실제 US provider adapter·정규장 read-only smoke·장기 soak는 다음 운영 단계다.
...
- Data Plane foundation과 US read-only supervisor·feature evidence gate의 fixture runtime은 구현됐지만 실제 US provider adapter, capability/entitlement registry 영속화와 health 수집, 전 종목 상시 raw stream, object-storage catalog 및 deterministic cross-source replay는 아직 없다. fixture receipt와 evidence-gated signal을 live entitlement, 운영 coverage 또는 수익성으로 해석하면 안 된다.
