...
- bounded Alpaca option-chain GET은 단일 underlying·만기·right의 raw response와 latest quote/trade·IV·Greeks·minute/daily/previous-daily bar snapshot을 보존한다. actual AAPL call 두 만기의 contract master·indicative chain을 exact identity `77/77`, `78/78`로 결합하고 최대 300초 as-of 경계의 OI·all-strike median-IV term structure와 network-free replay까지 확인했다. 실제 term structure에서 near/far median IV 차이를 분류하는 content-addressed shadow context도 발행했다. READY call/put surface와 exact SIP completed-minute spot receipt를 요구하는 matched-strike·absolute-delta bucket skew도 구현했지만 actual SIP GET은 `403`으로 차단돼 실제 skew 표본은 0건이다. provider-neutral futures roll security master와 as-of resolver는 fixture로 구현했고, actual CFTC TFF GET은 ES market-level weekly aggregate 두 시점의 다섯 category positioning context와 network-free replay를 확인했다. 두 immutable input과 reviewed binding을 결합하는 as-of positioning join은 actual CFTC에 fixture roll master를 결합한 증거까지 통과했다. 공식 Treasury actual GET은 14개 CMT maturity의 최신 두 일별 곡선과 network-free replay를 보존한다. 실제 CME·ICE licensed source adapter, 현재 계약 coverage, futures basis·curve, CFTC 장기 수정 이력·다중 market coverage와 derivatives performance trial·독립 Reviewer는 아직 없다. 이 제한된 slice를 실시간 파생상품 전체 수집이나 전략 성과로 해석하면 안 된다.
