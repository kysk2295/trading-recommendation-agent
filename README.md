...
**2026-07-19 multi-market shadow trial 업데이트:** global experiment ledger schema v5에 multi-market trial과 최대 두 event의 append-only chain을 추가했다. v1~v4 행은 재작성하지 않으며 current schema의 table/index/trigger exact-set도 Reader와 Writer가 검증한다. 첫 consumer인 KR `theme_leader_vwap_reclaim` day lane은 exact code-coupled shadow version, 다음 평일 09:00 KST 전 등록, 고정 no-entry baseline·20 sessions·30 signals·20bp 비용·결측 0·Reviewer gate를 요구한다. local CLI는 `register`와 `start`만 제공하며 fill, terminal, lifecycle, 계좌·주문 권한은 열지 않는다.
...
- KR day shadow 앞의 provider-neutral market gate는 세션·VI·단일가·거래정지·투자지정, 상하한가·+27% 근접, 5초 quote freshness와 bid/ask usability를 explicit evidence로 요구한다. KIS read-only raw adapter가 완료 분봉과 현재 가격·호가 evidence를 만들고, rank-1 leader의 exact session VWAP setup과 current-quote signal까지 fixture E2E로 연결됐다. day strategy는 exact lane과 code-coupled shadow version을 global ledger에 먼저 등록해야 한다. 실제 열린 정규장 read-only smoke, append-only trial/fill과 KRX 달력은 아직 남아 있다.
...
- global experiment ledger schema v5는 기존 v1~v4 행을 유지하면서 exact `StrategyLaneRef` 기반 multi-market hypothesis, strategy version, shadow trial과 started/terminal event chain을 별도 append-only table에 보존한다. KR theme Opportunity Manager와 `theme_leader_vwap_reclaim` day agent의 code-coupled shadow version은 각각 exact lane으로 등록되고, day agent는 고정 evidence budget의 daily trial 등록/start와 trial-bound conservative entry artifact까지 연결됐다. multi-market research source card, KR day exit/PnL·terminal, lifecycle과 독립 Reviewer는 아직 남아 있다.
