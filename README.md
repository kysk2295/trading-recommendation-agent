...
- bounded Alpaca option-chain GET은 단일 underlying·만기·right의 raw response와 latest quote/trade·IV·Greeks snapshot을 보존하고 local derivatives capability로 투영한다. 별도 option-contract security master의 actual AAPL GET은 고유 계약 77개와 OI 68개를 materialize했고, query-only exact identity join은 master 없는 snapshot을 차단하고 master coverage 전체일 때만 `READY` surface를 발행한다. 예약된 정규장 chain·surface smoke의 실제 terminal 결과, 시장 전체 옵션 coverage, OPRA entitlement, 다중 만기·strike의 IV·OI surface/skew/term structure, 선물 basis·curve·roll adapter와 derivatives 전략 agent는 아직 없다. 이 제한된 slice를 실시간 파생상품 전체 수집으로 해석하면 안 된다.
