...
- bounded Alpaca option-chain GET은 단일 underlying·만기·right의 raw response와 latest quote/trade·IV·Greeks·minute/daily/previous-daily bar snapshot을 보존한다. actual AAPL call 두 만기의 contract master·indicative chain을 exact identity `77/77`, `78/78`로 결합하고 최대 300초 as-of 경계의 OI·all-strike median-IV term structure와 network-free replay까지 확인했다. 시장 전체 옵션 coverage, OPRA entitlement, call/put parity, ATM·delta bucket skew와 per-strike volatility surface, 선물 basis·curve·roll adapter, derivatives 전략 agent는 아직 없다. 이 제한된 slice를 실시간 파생상품 전체 수집이나 전략 성과로 해석하면 안 된다.
