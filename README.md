...
**2026-07-23 M8 strict closeout→actual research admission 업데이트:** 2026-07-27 closeout 예약이 최소 watch cycle을 `1`로 설정했고 downstream actual research가 성공 receipt와 일반 strict marker만 검사해 이 완화 결과를 받아들일 수 있던 결손을 닫았다. actual research는 이제 closeout이 최소 `300` cycle을 요구하고 ranking/watch/retry/candidate 네 cardinality가 동일한 `300~390` 범위일 때만 plan·dataset·trial 단계로 진입한다. 최소치 완화, cardinality 불일치·누락·중복은 mutation 전에 차단된다. exact `ca78cf6` runtime으로 closeout·research·terminal audit을 다시 예약했으며 아직 clean 실제 세션 성공 증거는 아니다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-23-m8-open-handoff-provenance-schedule-ko.md)에 있다.
