...
- X·Reddit·Stocktwits의 운영용 공식 connector, provider별 수정·삭제·retention 수집과 social evidence pipeline이 없다. generic canonical correction/tombstone as-of replay와 Alpaca SIP `t/c/x` local fixture projection은 구현됐지만 실제 provider WebSocket 연속 수집이나 삭제 요청 이행을 뜻하지 않는다. 현재 뉴스·공시 수집을 전체 대체 데이터 coverage로 해석하면 안 된다.
