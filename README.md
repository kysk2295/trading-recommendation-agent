...
**2026-07-23 M8 정규장 handoff·provenance 분리 예약 업데이트:** 장전 collector의 sleep을 official 09:30 EDT open으로 상한 처리해 첫 정규장 scan 지연을 닫고, actual coordinator의 dataset producer SHA와 frozen strategy code version을 별도 필드로 분리했다. 초기 exact clean `f917ffe` runtime으로 2026-07-27 장전→정규장 watch, readiness, strict closeout과 연구 체인을 예약했고, cadence 수정 뒤 forward·progress는 `5ecad89`, strict closeout 경계 수정 뒤 closeout·research·audit은 `cccb799` runtime으로 교체했다. 품질 gate와 실패 cycle은 유지했고 executable Paper champion 두 개 전이라 Paper arm과 Allocation Manager는 열지 않았다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-23-m8-open-handoff-provenance-schedule-ko.md)에 있다.

**2026-07-23 M8 actual research terminal audit 업데이트:** 저장된 v2 manifest 파일의 trailing newline까지 포함한 실제 SHA와 in-memory canonical JSON SHA가 달라 trial provenance가 exact persisted artifact에 결속되지 않던 결손을 수정했다. 새 query-only audit은 immutable run plan, research receipt/report, catalog·CSV·dataset receipt, READY foundation 1~3개, persisted manifest, completed trial·walk-forward artifact와 독립 Reviewer를 전부 다시 읽어 exact identity와 cardinality를 검증하고 content-addressed mode-600 evidence를 발행한다. lifecycle·주문·allocation 권한은 모두 false다. 현재 exact clean `cccb799` runtime으로 2026-07-27 연구 작업과 연구 성공 영수증을 기다리는 16:50 EDT 후속 audit을 예약했다. 상세 근거는 [체크포인트](docs/checkpoints/2026-07-23-m8-open-handoff-provenance-schedule-ko.md)에 있다.
