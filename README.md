...
- global experiment ledger schema v3는 v2의 immutable `ResearchSource`·`ResearchHypothesisCard`에 더해 전략 버전별 `StrategyAuthorityBinding`을 보존한다. 공개 근거·주장·한계와 반증 기준을 local preregistration으로 보존하고 US legacy lane의 연구 좌표·최대 운영모드를 고정하지만, source card나 authority binding만으로 trial·Reviewer·lifecycle transition·champion·Paper 주문 권한은 생성하거나 변경하지 않는다. KR strategy registration은 multi-market experiment scope v2가 남아 있다.
