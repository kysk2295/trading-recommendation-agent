...
**2026-07-20 SEC EDGAR submissions read-only 업데이트:** M5의 첫 US regulatory source로 exact `https://data.sec.gov/submissions/CIK##########.json` GET을 추가했다. 선언된 연락처 `User-Agent`, no redirect, 45초 전체 deadline과 wire·decoded 각각 64 MiB cap을 강제하며 HTTP success/error wire bytes와 content encoding을 parser 전에 별도 mode-600 append-only SQLite receipt로 확정한다. recent filing은 accession별 immutable version chain으로 보존하고 동일 terminal 또는 orphan receipt replay는 provider와 설정 파일을 다시 열지 않는다. DB/report alias와 unsafe path는 provider 전에 거부하며 exact DDL·foreign key·correction observation 순서를 재검증한다. fixture success·correction·HTTP failure·transport failure와 CLI happy/replay, 전체 **2892 tests**, Ruff, basedpyright 0/0를 통과했으며 회사명·accession·CIK·원문·User-Agent는 report에 쓰지 않는다. `filings.files`는 개수만 기록하고 추가 history는 아직 수집하지 않으며 실제 SEC GET, broker/account/order mutation은 0건이다.
