...
**2026-07-26 인터랙티브 에이전트 대시보드 업데이트:** Observatory를 `개요 / 에이전트 / 계좌·PnL / 추천·근거` 작업 탭으로 재구성하고 에이전트별 목표 입력, queued/running/completed/failed 원장과 최종 응답을 이벤트 기반으로 연결했다. 공개 조회에는 계속 키가 필요 없고 명령은 Mac publisher가 발급한 일회용 링크로 연결된 Secure HttpOnly 기기 세션에서만 가능하다. 브라우저와 publisher는 주기적 HTTP polling을 하지 않으며 명시적 명령 1건만 로컬 Hermes 1회 실행으로 이어진다. 명령과 응답은 공개 viewer에 전송되지 않고 실제 자금 주문 권한도 추가하지 않는다.
