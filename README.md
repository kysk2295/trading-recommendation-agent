...
- Alpaca Paper GET 대사, 주문 스트림 control plane·heartbeat, raw-first `trade_updates` 영속화·격리·재시작 복구, 단일 WSS/Writer generation barrier와 활성 세션 기반 정규장/current-bar/portfolio 승인 게이트는 구현됐다. Alpaca WSS에는 replay cursor가 없으므로 연결 세대마다 REST 복구를 계속 요구한다. 실제 주문 제출, account activity 기반 체결 정정·취소 복구, 부분체결 보호주문, kill switch와 마감 전 강제청산은 아직 구현되지 않았다.
