...
- KR day shadow 앞의 provider-neutral market gate는 세션·VI·단일가·거래정지·투자지정, 상하한가·+27% 근접, 5초 quote freshness와 bid/ask usability를 explicit evidence로 요구한다. KIS read-only raw adapter와 private receipt store가 완료 분봉·현재가·호가 원문을 보존하고, 장중 child가 rank-1 leader의 exact session VWAP setup→current-quote signal→started trial의 보수적 entry까지 fixture E2E로 연결한다. day strategy는 exact lane과 code-coupled shadow version을 global ledger에 먼저 등록하고, official-shaped KIS 거래일 raw receipt에서 만든 snapshot으로 열린 session을 증명해야 한다. 실제 열린 정규장 KIS collector smoke, shadow exit polling과 재시작 가능한 일일 supervisor 연결은 아직 남아 있다.
